...
def _coerce_turn_input(user_input: Any) -> list[dict[str, str]]:
    """Preserve image inputs using the app-server's documented input union.

    URLs remain data, never interpolated into prompts or downloaded here.
    Local paths are accepted only in explicit app-server localImage items;
    a URL must not silently become a file read on the execution host.
    """
    if not isinstance(user_input, list):
        return [{"type": "text", "text": _coerce_turn_input_text(user_input)}]
    items = []
    for part in user_input:
        ...
        if kind in {"text", "input_text"}:
            ...
            if not isinstance(url, str) or not url.startswith(("https://", "http://", "data:image/")):
                raise ValueError("Codex image input requires an HTTP(S) or image data URL")
            items.append({"type": "image", "url": url})
        elif kind == "localImage":
            path = part.get("path")
            if not isinstance(path, str) or not path:
                raise ValueError("Codex local image input requires a path on the execution host")
...
class CodexAppServerSession:
    ...
    def __init__(
        ...
        *,
        ...
        initialized_client: Optional[CodexAppServerClient] = None,
    ) -> None:
        ...
        self._client: Optional[CodexAppServerClient] = initialized_client
        self._client_initialized = initialized_client is not None
    ...
    def run_turn(
        ...
        *,
        ...
        # Preserve rich content so computer screenshots actually reach Codex.
        try:
