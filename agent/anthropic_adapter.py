...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    return _new_sdk_client(_anthropic_sdk, kwargs)


def _new_sdk_client(sdk, kwargs):
    """Keep ambient credentials off the wire, even after SDK client copies.

    Adapted from NousResearch/hermes-agent's September 11 credential-isolation
    fixes. Clearing attributes alone is insufficient: with_options() re-runs
    the SDK constructor, which reads the unused credential from the environment.
    Omit default headers survive copies and suppress that credential on requests.
    """
    headers = dict(kwargs.get("default_headers") or {})
    if "api_key" in kwargs and "auth_token" not in kwargs:
        headers["Authorization"] = sdk.Omit()
    elif "auth_token" in kwargs and "api_key" not in kwargs:
        headers["X-Api-Key"] = sdk.Omit()
    client = sdk.Anthropic(**{**kwargs, "default_headers": headers})
    # Preserve this baseline's direct-client credential introspection contract.
    # The header omission, rather than these clears, protects reconstructed copies.
    if "api_key" in kwargs and "auth_token" not in kwargs:
