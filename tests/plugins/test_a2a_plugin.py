...
class TestTaskRpcHandlers:
    ...
    def test_tasks_cancel_resets_turns_for_context(self):
        ...
        adapter = _bare_adapter()
        from plugins.platforms.a2a.adapter import _forward_context_key
        context_key = _forward_context_key(str(adapter._agents[""].get("profile") or "default"), "", "peer", "ctx-loopy")
        for _ in range(4):
            adapter._turns.track(context_key)
        ...
        assert adapter._turns.track(context_key) == 1
...
class TestInboundRoundTrip:
    def test_authenticated_peers_cannot_read_cancel_or_resume_each_others_tasks(self, monkeypatch):
        ...
        seen = []
        def reply(event):
            seen.append((event.source.user_id, event.source.chat_id))
        ...
        async def run():
            ...
            try:
                ...
                first = await asyncio.to_thread(_post_json, base + "/", _send_body("hello", "same-context"), alice)
                await asyncio.to_thread(_post_json, base + "/", _send_body("hello", "same-context"), bob)
                assert seen[0][1] != seen[1][1]
                task_id = first["result"]["id"]
                for method in ["tasks/get", "tasks/cancel", "tasks/pushNotificationConfig/create"]:
                    response = await asyncio.to_thread(_post_json, base + "/", {
                        "jsonrpc": "2.0", "id": "query", "method": method,
                        "params": {"taskId": task_id, "pushNotificationConfig": {"url": "https://example.com/hook"}},
                    }, bob)
                    assert response["error"]["code"] == protocol.ERR_TASK_NOT_FOUND
                listed = await asyncio.to_thread(_post_json, base + "/", {
                    "jsonrpc": "2.0", "id": "list", "method": "tasks/list", "params": {},
                }, bob)
                assert len(listed["result"]["tasks"]) == 1
                assert listed["result"]["tasks"][0]["id"] != task_id
                own = await asyncio.to_thread(_post_json, base + "/", {
                    "jsonrpc": "2.0", "id": "own", "method": "tasks/get", "params": {"taskId": task_id},
                }, alice)
...
if '--resume' not in sys.argv:
    sid = 'sess-' + str(con.execute('SELECT count(*) FROM sessions').fetchone()[0] + 1)
    con.execute('INSERT INTO sessions (id, source, started_at, title) VALUES (?, ?, ?, ?)', (sid, 'a2a', time.time(), None))
    con.commit()
else:
...
""")
        ...
        con.close()
        from plugins.platforms.a2a.adapter import _forward_context_key
        assert title == "a2a-dev-" + _forward_context_key("dev", "dev", "peer", "ctx/unsafe value")

        # Authenticated callers and lossy display-slug collisions must never
        # resume each other's private threads, even with the same raw context.
        adapter._forward_to_profile(agent, "other-peer", "ctx/unsafe value", "separate peer")
        adapter._forward_to_profile(agent, "peer", "ctx-unsafe-value", "separate context")
        adapter._profile_sessions.clear()  # Restart: recover only the owned DB title.
        adapter._forward_to_profile(agent, "peer", "ctx/unsafe value", "resume owned")
        argv_lines = [json.loads(line) for line in calls.read_text().splitlines()]
        assert "--resume" not in argv_lines[2]
        assert "--resume" not in argv_lines[3]
        assert argv_lines[4][argv_lines[4].index("--resume") + 1] == "sess-1"
