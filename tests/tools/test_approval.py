...
class TestMcpConsentApprovalMode:
    def test_off_mode_bypasses_mcp_trust_prompt(self):
        with mock_patch.object(approval_module, "_YOLO_MODE_FROZEN", False), \
             mock_patch.object(approval_module, "is_current_session_yolo_enabled", return_value=False), \
             mock_patch.object(approval_module, "_get_approval_mode", return_value="off"), \
             mock_patch.object(approval_module, "prompt_dangerous_approval") as prompt:
            result = approval_module.request_elicitation_consent(
                "write-capable MCP tool",
                "explicitly untrusted server",
                surface="mcp-trust/orgo",
            )

        assert result == "accept"
        prompt.assert_not_called()

    def test_manual_mode_keeps_cli_mcp_consent_prompt(self):
        with mock_patch.object(approval_module, "_YOLO_MODE_FROZEN", False), \
             mock_patch.object(approval_module, "is_current_session_yolo_enabled", return_value=False), \
             ...
             mock_patch.object(approval_module, "prompt_dangerous_approval", return_value="deny") as prompt:
            result = approval_module.request_elicitation_consent(
                "write-capable MCP tool",
                "explicitly untrusted server",
                surface="mcp-trust/third-party",
            )
