...
def test_create_persists_explicit_failure_bound(worker_env):
    from hermes_cli import kanban_db as kb
    from tools import kanban_tools as kt
...
@pytest.mark.parametrize("value", [0, -1, True, 1.5, "3", "unlimited"])
def test_create_rejects_invalid_failure_bound(worker_env, value):
    from tools import kanban_tools as kt

    result = json.loads(kt._handle_create({
        "title": "invalid retry budget", "assignee": "peer", "max_retries": value,
    }))
