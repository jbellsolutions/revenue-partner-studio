...
def profile_dbs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_find_live_session_by_key", lambda _key, **_kwargs: None)
...
def test_resume_closes_profile_db_on_live_session_fast_path(profile_dbs, monkeypatch):
    ...
    monkeypatch.setattr("hermes_state.SessionDB", _factory)
    monkeypatch.setattr(server, "_find_live_session_by_key", lambda _key, **_kwargs: ("live-sid", {}))
    monkeypatch.setattr(
        server,
