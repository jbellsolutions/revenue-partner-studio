...
class TestTurnInputCoercion:
    @pytest.mark.parametrize("part,expected", [
        ({"type": "input_image", "image_url": "https://example.test/screen.png"},
    ...
    def test_rich_image_shapes(self, part, expected):
        assert _coerce_turn_input([part]) == [expected]

    @pytest.mark.parametrize("url", ["file:///etc/passwd", "/tmp/screen.png", "javascript:alert(1)", ""])
    def test_image_urls_never_become_implicit_local_file_reads(self, url):
        with pytest.raises(ValueError, match="image input"):
            _coerce_turn_input([{"type": "image", "url": url}])
...
class TestRunTurn:
    def test_rich_input_reaches_codex_without_losing_screenshot(self):
        client = FakeClient()
