        ...
        try:
            ...
            if params.get("paged"):
                ...
                rows = db.list_sessions_rich(
                    source=None, exclude_sources=list(deny), limit=limit + 1,
                    offset=offset, order_by_last_active=True, compact_rows=True,
                )
                return _ok(rid, {"sessions": [
                    {key: s.get(key) for key in ("id", "title", "preview", "started_at", "message_count", "source")}
                    for s in rows[:limit]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        with _session_resume_lock:
            live = _find_live_session_by_key(target, profile_home=profile_home or get_hermes_home())
            if live is not None:
