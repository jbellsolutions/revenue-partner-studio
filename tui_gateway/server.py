...
_LONG_HANDLERS = frozenset(
    {
        ...
        "session.compress",
        "session.list",
        # Studio imports and coordination must never block the WebSocket read
        # loop. Large profile transfers otherwise starve pings and status RPCs.
        "studio.operation",
        "studio.snapshot",
        "studio.a2a",
        "session.resume",
...
def _load_fallback_model():
    ...
    """
    if os.getenv('HERMES_STUDIO_RUNTIME') == '1':
        return []  # Studio reports provider failures; it never changes billing paths.
...
def _make_agent(
    ...
    if os.environ.get("HERMES_STUDIO_RUNTIME") == "1":
        from studio.cloud_runtime import tool_selection
        enabled_toolsets, disabled_toolsets = tool_selection(cfg, enabled_toolsets)
    return AIAgent(
        model=model,
...
def _find_live_session_by_key(session_key: str, *, profile_home=None) -> tuple[str, dict] | None:
    for sid, session in list(_sessions.items()):
        if session.get("_finalized"):
            continue
        if profile_home is not None:
            from hermes_constants import get_hermes_home
            actual = Path(session.get("profile_home") or get_hermes_home()).resolve()
            if actual != Path(profile_home).resolve():
                continue
            ...
            try:
                with session["history_lock"]:
                    if (_key and not session.get("running")
                            ...
                            and any(notification_inbox.has_pending(_home, key)
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        # Task-result turns use their durable inbox rather than a second crash
        # marker: two independent recovery paths could start duplicate turns.
        history = []
        try:
...
# Studio extends collaboration without replacing Hermes execution.
from studio.service import register as _register_studio
_register_studio(sys.modules[__name__])
