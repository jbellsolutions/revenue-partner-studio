...
export function LocalSetup({computers,select}:{computers:any[];select:(id:string)=>void}) {
  ...
  return <>
    ...
    {!setup&&id&&<div className="connection-card"><strong>{health?.runtimeConnected?'Hermes is responding':'Hermes needs attention'}</strong>
      <p>{health?.runtimeVersion==='screens-recovery-1'?'The compatible extension is connected. Repair can restore the saved connection if conversation recovery fails.':'Install the compatible companion to enable conversation recovery and selected-skill transfers.'}</p>
