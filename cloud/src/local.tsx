...
export function LocalSetup({computers,select}:{computers:any[];select:(id:string)=>void}) {
  ...
  return <>
    ...
    {!setup&&id&&<div className="connection-card"><strong>{health?.runtimeConnected?'Hermes is responding':'Hermes needs attention'}</strong>
      <p>{['screens-recovery-2','history-recovery-3','history-recovery-4','client-ready-screens-1'].includes(health?.runtimeVersion)?'The compatible extension is connected. Repair can restore the saved connection if conversation recovery fails.':'Install the compatible companion to enable conversation recovery and selected-skill transfers.'}</p>
