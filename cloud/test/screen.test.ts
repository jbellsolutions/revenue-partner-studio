...
test('unverified screen ownership stops automatic allocation and provides a repair action', async t => {
  ...
  t.mock.method(globalThis, 'fetch', async (_url: any, options: any) => {
    ...
    return new Response(JSON.stringify({ error: 'Existing screen has an unverified owner; work was preserved.' }), { status: 409 })
  })
  ...
  assert.ok(ui.getByText('Existing screen has an unverified owner; work was preserved.'))
  assert.deepEqual(methods, ['screen.open'])
  ...
  assert.deepEqual(methods, ['screen.open', 'screen.repair'])
