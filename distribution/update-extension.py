        ...
        try:
            with connect(url + '/api/ws?' + urlencode({'token': token}),
                ...
                while time.monotonic() < response_deadline:
                    ...
                    if result.get('sessionRecovery') and result.get('profileSettings') and result.get('historySearch') and result.get('screenSessions') and result.get('extensionVersion') == 'client-ready-screens-1': return
...
def update(configuration, source, apply=False, setup_job=None):
    ...
    result = {'computerId': cfg['computerId'], 'files': len(files), 'pendingTasks': queued,
              'ready': queued == 0, 'revision': 'client-ready-screens-1'}
