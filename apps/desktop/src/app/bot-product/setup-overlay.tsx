...
import type { DesktopOrgoConfig, DesktopTailscaleStatus } from '@/global'
import { getGlobalModelInfo } from '@/hermes'
import { Loader2 } from '@/lib/icons'
import { BOT_APP_NAME, isBotProduct } from '@/lib/product'
...
const ORGO_PROVISIONING_COPY: Record<OrgoProvisioningStage, { detail: string; title: string }> = {
  ...
  provisioning: {
    ...
    detail: 'Preparing the selected computer, checking Hermes, and applying your wallpaper. This can take a few minutes.'
  },
  'private-network': {
...
export function firstBotSoul(title: string, name: string): string {
  return [
    `# ${title}`,
    '',
    `You are ${title}, a persistent named agent (profile \`${name}\`).`,
...
export function BotSetupOverlay({
  ...
  const [existingComputerId, setExistingComputerId] = useState('')
  const [orgoStatus, setOrgoStatus] = useState<DesktopOrgoConfig | null>(null)
      ...
      if (status) {
        void status
          .then(config => {
            ...
            setOrgoStatus(config)

            if (!config.apiKeySet || !config.computerId) {
              return
            }
  ...
  // A connected Studio already owns its head agent remotely. Do not run the
  // inherited first-bot wizard or create a second local identity on reconnect.
  useEffect(() => {
    if (!enabled || !isBotProduct()) return
    let cancelled = false
    void requestGateway<{agents: {name: string}[]}>('studio.snapshot', {}).then(snapshot => {
      if (cancelled || !snapshot.agents.some(agent => agent.name === 'default')) return
      const ready: SetupState = {complete: true, skipped: false, step: 'ready'}
      writeSetup(ready)
      setSetup(ready)
    }).catch(() => undefined)
  ...
  if (!isBotProduct() || new URLSearchParams(window.location.search).get('workspace') === 'hermes' || !enabled || setup.complete || setup.skipped) {
    return null
  }
  ...
  const saveOrgo = async () => {
    ...
    try {
      ...
      if (!orgoStatus?.computerId && existingComputerId.trim()) {
        await window.hermesDesktop?.orgoDesktop.saveConfig({
          profile: 'default',
          apiKey: key,
          computerId: existingComputerId.trim()
        })
      } else {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-background/80 p-6 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl">
        <div className="text-xs uppercase tracking-wide text-muted-foreground">{BOT_APP_NAME}</div>
        <h1 className="mt-1 text-xl font-semibold">{heading}</h1>
        {setup.step === 'orgo' ? (
          <div className="mt-4 grid gap-3">
            <p className="text-sm text-muted-foreground">
              {orgoStatus?.computerId
                ? 'This Studio workspace is assigned to its Orgo computer. Enter the API key once to reconnect it; the computer will not change.'
                : 'Add Orgo to keep Hermes, every bot, and their memory running in the cloud. Hermes runs on the remote computer.'}
            </p>
            <Input
              disabled={busy}
            ...
            {!orgoStatus?.computerId ? (
              <label className="grid gap-1 text-sm">
                Existing Orgo computer ID (optional)
                <Input
                  disabled={busy}
                  onChange={event => setExistingComputerId(event.target.value)}
                  placeholder="Leave blank to create a computer"
                  value={existingComputerId}
                />
                <span className="text-xs text-muted-foreground">
                  Paste a computer ID from Orgo to connect this window to that computer and its Hermes profiles.
                </span>
              </label>
            ) : null}
            <Button aria-busy={busy} disabled={busy} onClick={() => void saveOrgo()}>
              ...
              {busy && orgoProvisioningStage
                ...
                : orgoKey.trim()
                  ? orgoStatus?.computerId
                    ? 'Reconnect saved computer'
                    : existingComputerId.trim()
                      ? 'Connect existing computer'
                      : 'Create cloud computer'
                  : orgoStatus?.computerId
                    ? 'Enter API key to reconnect'
