...
describe('usePromptActions sleep/wake session recovery', () => {
  ...
  it('resolves the owning profile across profiles when the session is not cached', async () => {
    // Match the scoped endpoint: a row on work is not returned by default.
    // afterEach resets this module-factory mock even when an assertion fails.
    setSessions(() => [])
    ...
    vi.mocked(getSession).mockImplementation(async (_id, profile) => {
      ...
      return sessionInfo({ id: STORED_SESSION_ID, profile: 'work' })
    })
