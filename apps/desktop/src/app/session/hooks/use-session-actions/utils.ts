...
export function chatMessagesEquivalent(a: ChatMessage, b: ChatMessage): boolean {
  if (
    ...
    !taskUpdatesEquivalent(a.taskUpdates, b.taskUpdates)
  ) {
    return false
  }
...
function taskUpdatesEquivalent(a: ChatMessage['taskUpdates'], b: ChatMessage['taskUpdates']): boolean {
  if (a === b) {
    return true
  }
  ...
  return a.every((row, index) =>
    (['task_id', 'board', 'text', 'title', 'worker', 'assignee', 'kind'] as const).every(
      key => row[key] === b[index][key]
    )
  )
}
...
export async function resolveStoredSession(storedSessionId: string): Promise<SessionInfo | undefined> {
  ...
  // past the sidebar's recent window). An unscoped REST call goes to the
  // PRIMARY backend, not the selected gateway. Pin the scope before awaiting:
  // a cold thread can open before the all-profile session list has refreshed.
  const activeKey = normalizeProfileKey($activeGatewayProfile.get())

  try {
    const session = await getSession(storedSessionId, activeKey)

    // The explicit desktop scope owns this result, even if a standalone remote
    // backend calls itself "default" or another click changes the active atom.
  ...
  // A reload resumes its URL before the sidebar necessarily loads profiles.
  // An empty cache is not proof that the launch profile is the only owner.
  // Discovery failures must reach bounded resume recovery, not become a false
  // "session deleted" result that sends the user to a blank/default chat.
  const knownProfiles = $profiles.get()
  const availableProfiles = knownProfiles.length ? knownProfiles : await refreshProfiles()

  const otherProfiles = availableProfiles
