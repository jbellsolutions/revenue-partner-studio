...
describe('chatMessagesEquivalent', () => {
  it('repaints changed task attribution but preserves equivalent resumed notices', () => {
    const base = {
      ...msg('1', 'system', 'done'),
      taskUpdates: [{ task_id: 't1', board: 'team', text: 'done', worker: 'analyst' }]
    }

    expect(chatMessagesEquivalent(base, { ...base, taskUpdates: base.taskUpdates.map(row => ({ ...row })) })).toBe(true)
    expect(
      chatMessagesEquivalent(base, { ...base, taskUpdates: [{ ...base.taskUpdates[0], worker: 'reviewer' }] })
    ).toBe(false)
    expect(chatMessagesEquivalent(base, { ...base, taskUpdates: undefined })).toBe(false)
  })
