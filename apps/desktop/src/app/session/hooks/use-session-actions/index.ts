...
import { clearConversationInterims, restoreConversationInterims } from '@/lib/conversation-interims'
...
function reconcileAuthoritativeMessages(
  ...
  const authoritative = restoreConversationInterims(storedSessionId ?? null, toChatMessages(authoritativeMessages))
...
export function useSessionActions({
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      let storedForProfile: Awaited<ReturnType<typeof resolveStoredSession>>
      ...
      try {
        ...
        await ensureGatewayProfile(storedForProfile?.profile, { isCurrent: isCurrentResume })
      } catch (error) {
        if (!isCurrentResume()) {
          return
        }

        // Profile readiness can fail before either transcript transport runs.
        // Do not leave a cleared thread outside the route's bounded recovery,
        // or fetch an unscoped fallback from the previously active profile.
        // A warm target may also have skipped the entry clear; its failure must
        // not leave the previous conversation painted under the new selection.
        if (!resumedSameSelectedSession) {
          setMessages([])
          setActiveSessionId(null)
          activeSessionIdRef.current = null
        ...
        notifyError(error, copy.resumeFailed)

        return
      }

      // A slow profile start may finish after a newer click or a fresh draft.
      // Never repaint its warm cache or dispatch its resume into that view.
      if (!isCurrentResume()) {
        return
      }
            ...
            : (() => {
                ...
                const resumedMessages =
                  resumed.messages_omitted && prefetchApplied && prefetchMatchesResumedSession
                    ...
                    : reconcileAuthoritativeMessages(resumed.messages, previousMessages, resumed, storedSessionId)
  ...
  const removeSession = useCallback(
    async (storedSessionId: string) => {
      ...
      try {
        ...
        clearConversationInterims(storedSessionId)
