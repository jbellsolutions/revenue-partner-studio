...
import { type GatewayEventPayload, textPart, toChatMessages } from '@/lib/chat-messages'
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        dispatchNativeNotification({
          actions: [
            { id: payload?.request_id ? `approve:${payload.request_id}` : 'approve', text: translateNow('notifications.native.approveAction') },
            { id: payload?.request_id ? `reject:${payload.request_id}` : 'reject', text: translateNow('notifications.native.rejectAction') }
          ],
        ...
        if (sessionId && payload?.kind === 'task_delivery') {
          ...
          if (notice) {
            flushQueuedDeltas(sessionId)
            updateSessionState(sessionId, state => {
              if (state.messages.some(message => message.id === notice.id)) {
                return state
              }
              // message.start precedes admission of the task-result turn.
              // Place its cause before the already-created response bubble.
              const pending = state.messages.findLastIndex(message => message.role === 'assistant' && message.pending)
              const messages = [...state.messages]
              messages.splice(pending < 0 ? messages.length : pending, 0, notice)
