...
import { rememberConversationInterims } from '@/lib/conversation-interims'
import {
...
export function useMessageStream({
  ...
  const finalizeInterimAssistantMessage = useCallback(
    (sessionId: string, text: string) => {
      ...
      rememberConversationInterims(updatedState.storedSessionId, updatedState.messages)
    },
    [updateSessionState]
  )
  ...
  const completeAssistantMessage = useCallback(
    (sessionId: string, text: string, responsePreviewed?: boolean, failure?: { error: string; partial: boolean }) => {
      ...
      // New backends persist tool-call commentary in SessionDB. This tiny
      // client overlay is the compatibility path for older remote backends
      // that emit message.interim but return only the final reply on resume.
      rememberConversationInterims(completedState.storedSessionId, completedState.messages)
