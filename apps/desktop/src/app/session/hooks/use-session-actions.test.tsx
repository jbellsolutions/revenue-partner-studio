...
describe('resumeSession failure recovery', () => {
  ...
  it('arms bounded recovery when the receiving profile cannot connect before resume', async () => {
    setSessions([storedSession({ profile: 'receiver', message_count: 2 })])
    vi.mocked(ensureGatewayProfile).mockRejectedValueOnce(new Error('receiver connection timed out'))
    const requestGateway = vi.fn(async () => ({}) as never)
  ...
  it.each([false, true])(
    'keeps newer selection when an older profile connection settles (failed: %s)',
    async failed => {
      const profileReady = deferred<void>()
      vi.mocked(ensureGatewayProfile)
        .mockClear()
        .mockReturnValueOnce(
          profileReady.promise.then(() => {
            if (failed) {
              throw new Error('obsolete profile failed')
            }
          })
        )
      setSessions([
        storedSession({ id: 'received', profile: 'receiver', message_count: 2 }),
        storedSession({ id: 'newer', profile: 'default', message_count: 2 })
      ])
      vi.mocked(getLatestSessionMessages).mockImplementation(
        async id =>
          ({
            session_id: id,
            messages: [{ role: 'assistant', content: `transcript for ${id}`, timestamp: 1 }]
          }) as never
      )
      ...
      const olderOpen = resume('received')
      await waitFor(() =>
        expect(vi.mocked(ensureGatewayProfile).mock.calls.some(([profile]) => profile === 'receiver')).toBe(true)
      )
      await resume('newer')
      profileReady.resolve()
      await olderOpen

      expect($selectedStoredSessionId.get()).toBe('newer')
      expect($activeSessionId.get()).toBe('runtime-newer')
      expect($resumeFailedSessionId.get()).toBeNull()
      expect(JSON.stringify($messages.get())).toContain('transcript for newer')
      expect(requestGateway.mock.calls.filter(([method]) => method === 'session.resume')).toEqual([
        ['session.resume', expect.objectContaining({ session_id: 'newer' })]
      ])
    }
  )

  it('does not expose the previous transcript when a warm target profile fails to connect', async () => {
    setSessions([storedSession({ profile: 'receiver', message_count: 2 })])
    setMessages([{ id: 'private-old', role: 'assistant', parts: [{ type: 'text', text: 'other agent history' }] }])
    setActiveSessionId('runtime-other-agent')
    const cached = createClientSessionState('stored-1')
    cached.messages = [{ id: 'cached-target', role: 'assistant', parts: [{ type: 'text', text: 'target history' }] }]
    vi.mocked(ensureGatewayProfile).mockRejectedValueOnce(new Error('receiver unavailable'))
    const requestGateway = vi.fn(async () => ({}) as never)

    await runResume(requestGateway, {
      runtimeIdByStoredSessionIdRef: { current: new Map([['stored-1', 'runtime-target']]) },
      sessionStateByRuntimeIdRef: { current: new Map([['runtime-target', cached]]) }
    })
...
describe('branchStoredSession desktop source tagging', () => {
  ...
  it('resolves and swaps to the parent profile when the branched session is not cached', async () => {
    ...
    vi.mocked(getSession).mockImplementation(async (_id, profile) => {
      if (profile !== 'work') {
        throw new Error('404: not on this profile')
      }

      return storedSession({ id: 'stored-parent', message_count: 1, profile: 'work' })
    })
    vi.mocked(getAllSessionMessages).mockResolvedValue({
      messages: [{ content: 'branch me', role: 'user', timestamp: 1 }],
      session_id: 'stored-parent'
