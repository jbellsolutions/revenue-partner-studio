...
describe('useDesktopIntegrations', () => {
  ...
  describe('profile-ready gate', () => {
    it('does not replace a reload destination before HashRouter publishes its pathname', () => {
      window.history.replaceState(null, '', '#/worker-thread')
      window.localStorage.setItem('hermes.desktop.lastRoute.profile.default', '/operator-thread')
      const sessions = [session({ id: 'operator-thread', profile: 'default' })]
      ...
      result.rerender({
        activeProfile: 'default',
        locationPathname: '/worker-thread',
        profileReady: true,
        resumeExhaustedSessionId: null,
        routedSessionId: 'worker-thread',
        sessions
      })
      expect(navigate).not.toHaveBeenCalled()
      expect(window.localStorage.getItem('hermes.desktop.lastRoute.profile.default')).toBe('/operator-thread')
    })
