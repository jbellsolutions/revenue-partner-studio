...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    if (!restoredRef.current) {
      // HashRouter may briefly report `/` while the actual URL already names
      // the conversation being reloaded. Do not replace that explicit route
      // with the launch profile's remembered inbox before the router catches up.
      const hashPath = window.location.hash.slice(1).split(/[?#]/, 1)[0]

      if (locationPathname === NEW_CHAT_ROUTE && hashPath.startsWith('/') && hashPath !== NEW_CHAT_ROUTE) {
        return
      }
