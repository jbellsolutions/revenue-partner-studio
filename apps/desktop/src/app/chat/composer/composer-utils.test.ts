...
describe('canSafelyCollapseComposer', () => {
  it('collapses only short drafts with no hard line break', () => {
    expect(canSafelyCollapseComposer('short single-line prompt')).toBe(true)
    expect(canSafelyCollapseComposer('first line\nsecond line')).toBe(false)
    expect(canSafelyCollapseComposer('a'.repeat(33))).toBe(false)
  })
})
