...
export function useComposerMetrics({
  ...
  const canSafelyCollapse = useAuiState(s => canSafelyCollapseComposer(s.composer.text))
  ...
  // a guessed character count. We only handle the cases the observer can't:
  // an explicit newline (expand before layout settles), plus empty or short
  // single-line drafts that are guaranteed to fit when collapsed. The short
  // threshold provides hysteresis, avoiding wrap/unwrap oscillation near the
  // actual layout boundary. We never read scrollHeight per keystroke.
  useEffect(() => {
    if (isEmpty || canSafelyCollapse) {
      setExpanded(false)
  ...
  }, [canSafelyCollapse, expanded, hasHardNewline, isEmpty])
