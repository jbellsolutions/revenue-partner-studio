...
type KanbanMessages = {
  ...
  col: Record<
    'archived' | 'blocked' | 'cancelled' | 'done' | 'ready' | 'review' | 'running' | 'scheduled' | 'todo' | 'triage',
    { label: string; help: string }
  >
  locked: { cancelled: string; review: string; running: string; scheduled: string }
  taskCancellation: {
    action: string
    title: string
    body: string
...
const en: KanbanMessages = {
  taskCancellation: {
    ...
    body: 'Stop this task and its verified local workers without retrying. History stays intact. This does not undo completed actions or cancel other tasks.',
    dismiss: 'Keep task',
    stopping: 'Requesting stop…',
    pending: 'Stop requested; worker cleanup is not yet confirmed.',
    stopped: 'Cancelled; verified workers stopped.',
  ...
  col: {
    ...
    cancelled: {
      label: 'Cancelled',
      help: 'Stop requested; no automatic retry. Open the task to check worker cleanup.'
    },
    archived: { label: 'Archived', help: 'Hidden from the default board view.' }
  },
  locked: {
    cancelled: 'Open the task and use Cancel task to confirm stopping its workers.',
...
const ja: KanbanMessages = {
  taskCancellation: {
    action: 'タスクを中止',
    title: 'このタスクを停止しますか？',
    body: 'このタスクと確認済みのローカルワーカーを、再試行せず停止します。履歴は保持されます。実行済みの操作は取り消されず、他のタスクは中止されません。',
    dismiss: 'タスクを維持',
    stopping: '停止を要求中…',
    pending: '停止を要求しました。ワーカーの終了はまだ確認できていません。',
    stopped: '中止済み。確認済みワーカーは停止しました。',
    retry: '停止を再試行'
  },
  nav: 'カンバン',
  ...
  col: {
    ...
    cancelled: {
      label: '中止',
      help: '停止要求済み。自動再試行はしません。タスクを開いてワーカーの終了を確認してください。'
    },
    archived: { label: 'アーカイブ', help: 'デフォルトのボード表示から非表示。' }
  },
  locked: {
    cancelled: 'タスクを開き「タスクを中止」でワーカーの停止を確認してください。',
...
const zh: KanbanMessages = {
  taskCancellation: {
    action: '取消任务',
    title: '停止此任务？',
    body: '停止此任务及已核实身份的本地工作进程，不再重试。保留历史记录，不撤销已完成的操作，也不取消其他任务。',
    dismiss: '保留任务',
    stopping: '正在请求停止…',
    pending: '已请求停止，尚未确认工作进程已结束。',
    stopped: '已取消，已核实的工作进程已停止。',
    retry: '重试停止'
  },
  nav: '看板',
  openBoard: '看板：打开面板',
  ...
  col: {
    ...
    cancelled: { label: '已取消', help: '已请求停止，不会自动重试。打开任务以查看进程清理状态。' },
    archived: { label: '已归档', help: '从默认面板视图中隐藏。' }
  },
  locked: {
    cancelled: '打开任务并使用“取消任务”确认停止工作进程。',
...
const zhHant: KanbanMessages = {
  taskCancellation: {
    action: '取消任務',
    title: '停止此任務？',
    body: '停止此任務及已核實身分的本機工作程序，不再重試。保留歷史紀錄，不撤銷已完成的操作，也不取消其他任務。',
    dismiss: '保留任務',
    stopping: '正在要求停止…',
    pending: '已要求停止，尚未確認工作程序已結束。',
    stopped: '已取消，已核實的工作程序已停止。',
    retry: '重試停止'
  },
  nav: '看板',
  openBoard: '看板：開啟面板',
  ...
  col: {
    ...
    cancelled: { label: '已取消', help: '已要求停止，不會自動重試。開啟任務以查看程序清理狀態。' },
    archived: { label: '已封存', help: '從預設面板檢視中隱藏。' }
  },
  locked: {
    cancelled: '開啟任務並使用「取消任務」確認停止工作程序。',
...
// Existing board copy falls back to English in Arabic; translate every new
// cancellation control without claiming the whole plugin is translated.
const ar: KanbanMessages = {
  ...en,
  taskCancellation: {
    action: 'إلغاء المهمة',
    title: 'إيقاف هذه المهمة؟',
    body: 'إيقاف هذه المهمة وعملياتها المحلية التي تم التحقق منها دون إعادة المحاولة. يُحفظ السجل. لا يتراجع هذا عن الإجراءات المنجزة ولا يلغي المهام الأخرى.',
    dismiss: 'الإبقاء على المهمة',
    stopping: 'جارٍ طلب الإيقاف…',
    pending: 'طُلب الإيقاف؛ لم يتأكد انتهاء العمليات بعد.',
    stopped: 'أُلغيت المهمة وتوقفت العمليات التي تم التحقق منها.',
    retry: 'إعادة محاولة الإيقاف'
  },
  col: {
    ...en.col,
    cancelled: { label: 'ملغاة', help: 'طُلب الإيقاف دون إعادة محاولة تلقائية. افتح المهمة للتحقق من انتهاء العمليات.' }
  },
  locked: { ...en.locked, cancelled: 'افتح المهمة واستخدم إلغاء المهمة لتأكيد إيقاف عملياتها.' }
}

export const KANBAN_LOCALES: PluginLocaleBundles = { en, ja, zh, ar, 'zh-hant': zhHant }
