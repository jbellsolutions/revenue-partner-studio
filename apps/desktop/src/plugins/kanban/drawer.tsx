...
import { TaskCancellation } from './cancellation'
...
function workerSessionId(run: KanbanRun): unknown {
...
export function TaskDrawer({
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      // A confirmation dialog owns Escape while open; one gesture must not
      // dismiss both the confirmation and the task underneath it.
      if (
        event.key === 'Escape' &&
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4" data-selectable-text="true">
        {errorMessage ? (
          ...
          <div className="flex flex-col gap-4 text-sm">
            <TaskCancellation key={`${slug}:${task.id}`} task={task} />
                    ...
                    {detail.runs.map(run => {
                      ...
                      return (
                        <li className="flex flex-col gap-0.5 text-[0.71rem]" key={run.id}>
                          ...
                          <WorkerThreadLink profile={run.profile} sessionId={workerSessionId(run)} />
                        </li>
                      )
                    })}
                  </ul>
