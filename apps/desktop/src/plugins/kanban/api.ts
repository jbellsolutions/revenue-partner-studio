...
/** Stop intent must never nudge the dispatcher or fall back to reclaim/retry.
 * The caller snapshots the board before an asynchronous confirmation/write. */
export const cancelTask = (id: string, slug: string) =>
  call<{ ok: boolean; status: string; worker_stopped: boolean }>(
    withBoard(`/tasks/${encodeURIComponent(id)}/cancel`, {}, slug),
    { method: 'POST', body: {} }
  )
