import { StudioBrand } from '@/app/bot-product/studio-identity'
import { StudioCollaboration, closeStudioConversation } from '@/app/bot-product/studio-collaboration'
/**
...
function botAppearance(name, meta) {
  return {
    ...
    image: meta?.image || (isBotProduct() && name === 'default' ? './studio-icon.png' : null)
  }
}
...
function rosterRequestParams() {
  return new URLSearchParams(window.location.search).get('workspace') === 'hermes'
...
function useRoster() {
  return useQuery({
    queryKey: ROSTER_KEY,
    queryFn: () => host.request('profiles.list', rosterRequestParams()),
...
// Explicit per-agent threads are navigation choices, not replacement canonical
// inboxes. The messenger's recovery loop must not snap them back to Bot Chat.
let explicitThreadProfile = null
let navigationIntentEpoch = 0
let navigationIntentTarget = ''
let workspaceRestoreAttempted = false
const enqueueSessionNavigation = createBotSessionNavigationQueue()
let pluginDisposed = false

function claimNavigationIntent(target) {
  navigationIntentEpoch += 1
  navigationIntentTarget = target
  if (/^(bot|group):/.test(target)) {
    Promise.resolve(pluginCtx?.storage?.set?.('last-conversation', target)).catch(() => undefined)
  }

  return { epoch: navigationIntentEpoch, target }
}

async function restoreWorkspaceConversation(roster) {
  const epoch = navigationIntentEpoch
  const saved = await Promise.resolve(pluginCtx?.storage?.get?.('last-conversation')).catch(() => null)
  // A click during storage/connection recovery always wins over the saved selection.
  if (pluginDisposed || navigationIntentEpoch !== epoch || navigationIntentTarget) return
  if (typeof saved === 'string' && saved.startsWith('group:')) {
    const group = $botGroups.get()[saved.slice(6)]
    if (group) return openBotGroup(group)
  }
  const name = typeof saved === 'string' && saved.startsWith('bot:') ? saved.slice(4) : 'default'
  const bot = roster.find(row => row.name === name) || roster.find(row => row.name === 'default')
  if (bot) return openBotChat(bot)
}
...
function queueSessionNavigation(storedId, profile, intent, { surfaceErrors = false } = {}) {
  const isCurrent = () => isCurrentNavigationIntent(intent)
  const run = enqueueSessionNavigation(
    async () => {
      if (typeof host.openSession !== 'function') {
        throw new Error('Session navigation is unavailable in this desktop runtime.')
      }

      await host.openSession(storedId, { profile, isCurrent })
    },
    isCurrent
  )

  // Canonical inbox recovery runs in the background. Explicit user selections
  // must reject into AgentThreads/ShareHistory's existing error-and-retry UI.
  return surfaceErrors ? run : run.catch(() => false)
}
...
function routedStoredSessionId() {
  ...
  try {
    // Core knows contributed pages such as /kanban. Treating them as stored
    // session IDs makes the canonical-chat recovery loop hijack the page.
    return routeSessionId(hash)
  } catch {
    return null
  }
}
...
function snapToCanonicalIfStray() {
  if (explicitThreadProfile) return
  if (
    pinningCanonical ||
  ...
  // A routed thread is an explicit destination, including after a reload when
  // the in-memory explicitThreadProfile flag has not survived. Recover the
  // canonical inbox only from a blank chat, never over an existing thread.
  if (stored) {
    return
  }
...
function onBotModeNewSessionRequested(event) {
  if (!['slash', 'agent-thread'].includes(event?.detail?.source) || !botsMessengerActive()) {
    return
  }
  ...
  pendingExplicitNewSession = {
    standaloneThread: event.detail.source === 'agent-thread' || Boolean(explicitThreadProfile),
...
function displayName(bot, meta) {
  ...
  // that reads like nobody bothered. In the dedicated product, use the app's
  // actual name so the user's primary bot is findable in the roster; generic
  // Hermes Desktop keeps its established Hermes label.
  if ((bot.name || '').trim().toLowerCase() === 'default' && !bot.title) {
    return isBotProduct() ? 'Head of Operations' : 'Hermes'
  }
...
function composeSoul({ name, title, description, roster, customSoul }) {
  const runtimePlacement = [
    '## Runtime placement',
    '',
    'Your normal local tools run on the machine hosting the Hermes backend.',
    'When Orgo tools are configured, that host is the bound Orgo computer and',
    "the user's Mac is only a remote control surface. Never use the client",
    'browser or filesystem implicitly, and never claim you run on the client Mac.'
  ].join('\n')
...
async function openBotChat(bot, { intent: suppliedIntent = null, preserveDraft = false, quiet = false } = {}) {
  closeStudioConversation()
  explicitThreadProfile = null
...
function onFirstBotProfile(event) {
  ...
  void (async () => {
    ...
    try {
      const response = await host.request('profiles.list', { include_sessions: false })
...
async function openAgentThread(profile, id) {
  closeStudioConversation()
  const intent = claimNavigationIntent(`thread:${profile}:${id}`)
  explicitThreadProfile = profile
  pendingExplicitNewSession = null
  $newConversation.set(null)
  $activeGroupId.set(null)
  $selectedBot.set(profile)
  lastMessengerBot = profile
  seedBotComposerModel(profile)
  await queueSessionNavigation(id, profile, intent, { surfaceErrors: true })
}

function beginAgentThread(profile) {
  explicitThreadProfile = profile
  $activeGroupId.set(null)
  $newConversation.set(null)
  $selectedBot.set(profile)
  lastMessengerBot = profile
  onBotModeNewSessionRequested({ detail: { source: 'agent-thread' } })
}
...
async function createOrActivateDraftGroup(draft, roster) {
  await host.request('studio.operation', { operation: 'create_group', name: draft.id, title: groupTitle(draft.participantIds, roster), members: draft.participantIds })
...
function existingHermesGroupEnvelope(group, userText, roster) {
...
function groupRoutingEnvelope(group, userText, roster) {
  if (new URLSearchParams(window.location.search).get('workspace') === 'hermes') {
    return existingHermesGroupEnvelope(group, userText, roster)
  }
  return [BOT_GROUP_CONTEXT_START,
    `This is the Studio project group ${group.title} (id: ${group.id}).`,
    `Members: ${group.participantIds.join(', ')}.`,
    'Use studio_team to send work to these real Hermes agents. Inspect actual results before summarizing them.',
    'Speak under your own identity. Agent replies appear in the recorded Exchanges view; do not emit attributed reply markers.',
    BOT_GROUP_CONTEXT_END].join('\n')
}
      ...
      jsxs('div', {
        ...
        children: [
          ...
          jsx('div', {
            ...
            children: last?.preview || bot.description || (Object.hasOwn(bot, 'last_session') ? 'No conversations yet — say hi' : 'Open conversation')
          })
        ]
      })
    ]
...
function ProviderMark({ className, id, size = 16 }) {
  if (id === 'grok') return jsx(GrokMark, { className, size })
  if (id === 'openrouter') return jsx(Codicon, { className, name: 'globe', size: size ? `${size}px` : '1rem' })
  return jsx(OpenAiMark, { className, size })
}

// ── provider switch (Grok / OpenAI / OpenRouter, per bot) ─────────────────────────────────

/** The providers Bot Mode offers in the chat header, mirroring the Orgo
...
const BOT_PROVIDER_CHOICES = [
  { candidates: ['openai-codex'], id: 'codex', label: 'Codex' },
  { candidates: ['anthropic'], id: 'claude', label: 'Claude' },
  { candidates: ['openrouter'], id: 'openrouter', label: 'OpenRouter' },
  { candidates: ['ollama-cloud'], id: 'ollama-cloud', label: 'Ollama Cloud' }
]
...
function resolveProviderChoice(rows, candidates) {
  ...
  for (const slug of candidates) {
    ...
    if (row && model && row.authenticated !== false) {
                    ...
                    children: [
                      ...
                      jsx('div', {
                        ...
                        children: railChoice.ready
                          ...
                          : 'Use Model & Provider Settings to connect'
                      })
                    ]
                  }),
                  railChoice.ready
                    ...
                    : jsx('button', {
                        className:
                          'w-full rounded-lg px-2 py-3 text-left text-[13px] text-(--ui-accent) hover:bg-(--ui-control-active-background)/60',
                        onClick: () => {
                          haptic('tap')
                          host.navigate('/settings?tab=models')
                        },
                        type: 'button',
                        children: 'Open Model & Provider Settings'
                      }),
  ...
  if (!conversation) {
    ...
    return jsxs('div', {
      ...
      children: [
        jsx(
          BotFace,
          { color, image, mood: 'idle', name: bot.name, shape, size: 18 }, 'direct-bot-face'),
        jsx(
          'span',
          {
...
/** Korgo is a focused shell, not a reduced runtime. These links deliberately
 * open the existing Hermes management pages against whichever backend owns the
 * active desktop connection (local or remote). */
const BOT_MANAGEMENT_LINKS = [
  { icon: 'book', label: 'Skills', path: '/skills?tab=skills' },
  { icon: 'library', label: 'Skills Hub', path: '/skills?tab=hub' },
  { icon: 'tools', label: 'Capabilities', path: '/skills?tab=toolsets' },
  { icon: 'extensions', label: 'Plugins & MCP', path: '/skills?tab=mcp' },
  { icon: 'files', label: 'Artifacts', path: '/artifacts' },
  { icon: 'organization', label: 'Profiles', path: '/profiles' },
  { icon: 'gear', label: 'Settings', path: '/settings' }
]

function FooterRow({ icon, label, onClick }) {
  return jsxs('button', {
    className: cn(
    ...
    children: [
      jsx('span', {
        ...
        children: jsx(Codicon, { name: icon, size: '0.7rem' })
      }),
      jsx('span', { className: 'truncate text-[0.78rem] font-medium leading-4', children: label })
    ]
  })
}
...
async function importLocalSkills() {
  try {
    const result = await globalThis.hermesDesktop?.orgoDesktop?.syncSkills?.()

    if (!result || result.cancelled) return
    host.notify({
      kind: 'success',
      message: `Imported ${result.skillCount} skills (${result.fileCount} files) to the Korgo computer.`
    })
    host.navigate('/skills?tab=skills')
  } catch (error) {
    host.notifyError(error, 'Could not import local skills')
  }
}

/** The pane's bottom shelf keeps the complete Korgo runtime within one click:
 * Composio-connected apps, native Hermes management pages, and the active
 * bot's model. */
function PaneFooter({ threadProfile }) {
  return jsxs('div', {
    className: 'studio-footer shrink-0 px-3 py-3',
    children: [
      jsx(FooterRow, { icon: 'plug', label: 'Connected apps', onClick: () => host.connectors?.open?.() }),
      jsxs('details', { children: [
        jsx('summary', { className: 'studio-settings-summary', children: 'Workspace settings' }),
        jsx(TeamTasksButton, { onNavigate: path => host.navigate(path) }),
        jsx(FooterRow, { icon: 'cloud-upload', label: 'Import skills', onClick: () => void importLocalSkills() }),
        jsx('div', { className: 'grid grid-cols-2 gap-1', children: BOT_MANAGEMENT_LINKS.map(link => jsx(FooterRow, {
          icon: link.icon, label: link.label, onClick: () => host.navigate(link.path)
        }, link.path)) }),
        jsx(ComputerWindows, {}),
        jsx(AgentThreads, {profile: threadProfile, request: host.request, onOpenThread: openAgentThread,
          onNewThread: beginAgentThread, onCreateAgent: () => host.navigate('/profiles')}),
...
function BotsPane() {
  const threadProfile = useValue($selectedBot) || 'default'
  ...
  useEffect(() => {
    ...
    const snapshot = filterDeletedRoster(live)
      ...
      .sort((a, b) => {
        ...
        const activityOfSnapshot = bot => {
          const created = meta[bot.name]?.created || bot.ui_meta?.['hermes-bots']?.created || 0
          const lastMsg = (bot.last_session?.last_active || 0) * 1000

          return Math.max(created, lastMsg)
        }

        return activityOfSnapshot(b) - activityOfSnapshot(a)
      })

    $lastRoster.set(snapshot)
    mergeServerMeta(snapshot)
    pullServerAvatars(snapshot)
    trackInboundActivity(snapshot)
    pruneGroupsAgainstRoster(snapshot)
    if (!workspaceRestoreAttempted && snapshot.length && new URLSearchParams(window.location.search).get('workspace') === 'hermes') {
      workspaceRestoreAttempted = true
      void restoreWorkspaceConversation(snapshot).catch(error => host.notifyError(error, 'Could not reopen the saved conversation'))
    }
  }, [live])
  ...
  return jsxs('div', {
    ...
    children: [
      isBotProduct() ? jsx(StudioBrand, {connected: gatewayUp}, 'studio-brand') : null,
      jsx('div', {
        className: 'studio-search px-4 pb-4 pt-1',
        children: jsxs('label', {
          className:
          ...
          children: [
            jsx(Codicon, { name: 'search', size: '0.75rem' }, 'search-icon'),
            jsx('input', {
              value: search,
              className:
      ...
      isBotProduct() ? jsx('div', {className: 'studio-nav-label', children: 'BOTS'}, 'bots-label') : null,
      staleNotice
        ? jsx('div', {
            className:
        ...
        : null,
      draft?.participantIds?.length ? jsx(ConversationDraftRow, { draft, roster }, 'pane-draft') : null,
      isLoading && !roster.length && !groupRows.length
        ...
        : error && !roster.length && !groupRows.length
          ...
          : roster.length === 0 && groupRows.length === 0
            ? jsx(EmptyState, {
                icon: 'hubot',
                title: isBotProduct() && !gatewayUp ? 'Waiting for Orgo' : 'No agents yet',
                description: isBotProduct() && !gatewayUp ? 'Your agents will appear when the remote workspace reconnects.' : 'Create your first teammate.'
              }, 'pane-body')
            : jsx(ScrollArea, {
                className: cn(
                ...
                children: jsxs('div', {
                  ...
                  children: [
                    ...
                    jsx(PinnedStrip, { bots: pinnedRoster, onEdit: setEditing, onDelete: setDeleting }, 'pinned-strip'),
                    jsx('div', {
                      className: 'flex min-h-32 flex-col',
                      children: [
                        jsx(CreateNewRow, {}, 'create-new'),
                        jsx('div', {
      ...
      isBotProduct() ? jsx(StudioCollaboration, {connected: gatewayUp, request: host.request, onOpenThread: openAgentThread}, 'studio-groups') : null,
      jsx(PaneFooter, {threadProfile}, 'pane-footer'),
...
export default {
  ...
  register(ctx) {
    ...
    const releaseThreadOpener = registerAgentThreadOpener(openAgentThread)
    ctx.onDispose?.(releaseThreadOpener)
    ...
    style.textContent = `
        ...
        /* Message renderers contain their own markdown type scale, so changing
           ...
           complete Bot Mode bubble subtree to the comfortable messenger
        ...
        body:has(div.absolute:not([data-pane-hidden]) [data-hermes-bots-pane]) [data-chat-surface] {
          --conversation-turn-gap: 1.25rem;
          --composer-control-size: 2rem;
          --composer-control-primary-size: 2rem;
          --composer-input-min-height: 2.75rem;
          --composer-surface-pad-x: 0.75rem;
          --composer-surface-pad-y: 0.625rem;
    ...
    const stopSessionInfoListener = host.onEvent('session.info', event => {
      ...
      if (
        explicitNew &&
        (!explicitNew.standaloneThread || runtimeId === activeRuntime) &&
        eventProfile === explicitNew.name &&
        storedId !== explicitNew.previousChat
      ) {
        if (explicitNew.standaloneThread) {
          // Keep the canonical A2A/inbox target and every old title unchanged.
          pendingExplicitNewSession = null
          return
        }
      ...
      if (!explicitThreadProfile && binding.action !== 'bind' && binding.action !== 'keep') {
      ...
      data: {
        handler: async draft => {
          ...
          if (pendingDraft) {
            ...
            activeGroup = await createOrActivateDraftGroup(pendingDraft, $lastRoster.get())
          }
