 ...
 *  entry (a url, …) renders as an accessible inline control that runs it on
 *  click or keyboard activation; everything else is inert text. `onClick`
 *  overrides for chips that resolve their target themselves (session, which
 *  needs the async navigator). */
const DirectiveChip: FC<{
  ...
  return activate ? (
    <span {...props} onClick={activate} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); activate(); } }} role="button" tabIndex={0}>
      {body}
    </span>
  ) : (
