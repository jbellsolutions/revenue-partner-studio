...
describe('thinking indicator is tail-only', () => {
  it('shows the loading indicator on a running placeholder at the tail', async () => {
    ...
    expect(container.querySelector('[data-slot="aui_compact-run-activity"]')).toBeTruthy()
  })
  ...
  it('never shows an indicator on a stale running message mid-transcript', async () => {
    ...
    expect(container.querySelector('[data-slot="aui_compact-run-activity"]')).toBeNull()
    expect(container.querySelectorAll('[data-slot="aui_assistant-message-root"]')).toHaveLength(1)
  })
})
