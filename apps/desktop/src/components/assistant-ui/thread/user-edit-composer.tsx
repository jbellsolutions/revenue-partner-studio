...
export const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const aui = useAui()
  const storedDraft = useAuiState(s => s.composer.text)
  const draft = stripBotGroupContext(storedDraft)
  ...
  // The durable group-routing envelope is model-visible, not human-editable.
  // Bot Mode middleware appends a fresh envelope with the group's current
  // participants when this edit is submitted again.
  useEffect(() => {
    if (storedDraft !== draft) {
      aui.composer().setText(draft)
    }
  }, [aui, draft, storedDraft])
  ...
  return (
    <ComposerPrimitive.Root className="contents" data-slot="aui_edit-composer-root">
      <StickyHumanMessageContainer>
        <div
          className="composer-human-message-container human-execution-message-top relative ml-auto flex w-full max-w-[70%] items-start rounded-md bg-(--ui-chat-surface-background)"
        ...
        >
          ...
          <div
            className={cn(
              USER_BUBBLE_BASE_CLASS,
              'ui-prompt-input__container relative ml-0 w-full max-w-full border border-(--ui-stroke-secondary) data-[expanded=true]:min-h-20',
