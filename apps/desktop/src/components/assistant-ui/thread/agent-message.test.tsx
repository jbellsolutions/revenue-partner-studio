...
import { AGENT_MESSAGE_RE, parseAgentMessageSender } from './user-message'
...
describe('agent message detection', () => {
  ...
  it('uses exact participant profile handles for group chat avatar stacks', () => {
    const m = AGENT_MESSAGE_RE.exec(
      'Message from group chat Dewey, New Bot, New Bot [profiles=dewey,new-bot,new-bot-2]: hello'
    )

    const identity = parseAgentMessageSender(m?.[1] ?? '')

    expect(identity.sender).toBe('group chat Dewey, New Bot, New Bot')
    expect(identity.groupHandles).toEqual(['dewey', 'new-bot', 'new-bot-2'])
  })

  it('derives profile handles for group notices created before profile metadata was added', () => {
    const identity = parseAgentMessageSender('group chat Dewey, New Bot')

    expect(identity.groupHandles).toEqual(['dewey', 'new-bot'])
  })
