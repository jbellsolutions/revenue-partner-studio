...
export function GatewayConnectingOverlay() {
  ...
  if (isBotProduct() && !previewing) {
    return <aside role="status" className="fixed bottom-5 left-1/2 z-(--z-connecting) w-[min(28rem,calc(100%-2rem))] -translate-x-1/2 rounded-xl border border-(--ui-stroke-secondary) bg-(--ui-chat-surface-background) px-4 py-3 shadow-lg">
      <p className="text-sm font-medium">Connecting to Orgo</p>
      <p className="mt-1 text-xs text-(--ui-text-secondary)">The remote workspace is not connected yet. Computer settings remain available while the connection retries.</p>
      <Button className="mt-2" variant="outline" size="sm" onClick={requestOrgoDesktopSettings}>Connection settings</Button>
    </aside>
  }
