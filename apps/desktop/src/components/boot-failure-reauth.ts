...
// Extract only the exact one-time authorization URL emitted by Tailscale SSH.
// The renderer passes this to openExternal, so fail closed on lookalike hosts,
// alternate protocols, query strings, and unexpected paths.
export function tailscaleSshCheckUrl(error: string | null | undefined): string {
  const match = String(error || '').match(TAILSCALE_SSH_CHECK_URL_RE)
  ...
  try {
    ...
    return url.protocol === 'https:' && url.hostname === 'login.tailscale.com' && /^\/a\/[A-Za-z0-9_-]+$/.test(url.pathname)
      ? url.toString()
      : ''
  } catch {
    return ''
...
export function sshFailureMessage(
  ...
  if (tailscaleSshCheckUrl(raw)) {
    return copy.sshErrTailscaleCheck || raw
  }
