...
describe('sshFailureMessage', () => {
  ...
  it('surfaces a Tailscale identity check before generic SSH auth copy', () => {
    const ssh = config({ mode: 'ssh', sshHost: 'box', remoteUrl: '' })

    const error =
      'Tailscale needs a quick identity check. Complete it: https://login.tailscale.com/a/fixture-token'

    expect(tailscaleSshCheckUrl(error)).toBe('https://login.tailscale.com/a/fixture-token')
    expect(tailscaleSshCheckUrl('https://login.tailscale.com.evil.example/a/fixture-token')).toBe('')
    expect(sshFailureMessage(ssh, error, { sshErrTailscaleCheck: 'localized check' })).toBe('localized check')
  })
})
