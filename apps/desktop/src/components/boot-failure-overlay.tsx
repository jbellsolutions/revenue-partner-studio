...
import { ComputerWindows } from '@/app/bot-product/workspace-controls'
...
type BusyAction = 'local' | 'repair' | 'retry' | 'signin' | 'tailscale' | null
...
export function BootFailureOverlay() {
  ...
  const tailscaleReturnCleanup = useRef<(() => void) | null>(null)
  ...
  useEffect(
    () => () => {
      tailscaleReturnCleanup.current?.()
    },
    []
  )
  ...
  const tailscaleCheck = tailscaleSshCheckUrl(boot.error)

  // Opening the browser must be an explicit click; when focus returns after the
  // short Tailscale check, reconnect automatically so the user does not land
  // back on the same failure card and have to infer that Retry is now safe.
  const authorizeTailscaleAndReconnect = async () => {
    if (!tailscaleCheck) {
      return
    }
    ...
    const cleanup = () => {
      ...
      tailscaleReturnCleanup.current = null
    }
    ...
    tailscaleReturnCleanup.current?.()
    tailscaleReturnCleanup.current = cleanup

    try {
      await window.hermesDesktop?.openExternal(tailscaleCheck)
      armed = true
    } catch (err) {
      cleanup()
      notifyError(err, t.boot.failure.tailscaleCheckFailed)
    } finally {
      setBusy(null)
    }
  }
  ...
  if (tailscaleCheck && connectionConfig?.mode === 'ssh') {
    actions = [
      {
        key: 'tailscale',
        label: copy.authorizeTailscaleAndReconnect,
        onClick: () => void authorizeTailscaleAndReconnect(),
        icon: <ExternalLink />,
        busy: 'tailscale'
      },
  ...
  if (isBotProduct()) {
    actions = actions.filter(action => action.key !== 'local' && action.key !== 'repair')
    if (!actions.some(action => action.key === 'settings')) actions.unshift(settingsAction)
    hint = 'Check the connection to the dedicated Orgo computer, then reconnect. Agent work is owned by the remote workspace.'
  }
  ...
  if (isBotProduct()) {
    return <aside className="fixed bottom-5 right-5 z-(--z-setup) w-80 rounded-xl border border-white/10 bg-[#22242a] p-4 text-[#eceef4] shadow-xl" role="status">
      <p className="text-[13px] font-semibold">Orgo connection needed</p>
      <p className="mt-2 text-xs leading-5 text-[#a0a5b3]">The remote computer is not reachable from this Mac yet.</p>
      <div className="mt-3 flex gap-2"><Button size="sm" onClick={() => setView('connect')}>Connection settings</Button><Button size="sm" variant="ghost" onClick={() => void retry()}>Retry</Button></div>
      <details className="mt-3 text-[11px] text-[#a0a5b3]"><summary className="cursor-pointer">Connection details</summary><p className="mt-2 break-words">{sshFailureMessage(connectionConfig, boot.error, t.settings.gateway)}</p><Button size="sm" variant="ghost" onClick={openLogs}>Open logs</Button></details>
    </aside>
  }
        ...
        <div className="flex items-start gap-3 px-5 py-4">
          ...
          <div>
            <h2 className="text-[0.9375rem] font-semibold tracking-tight">
              {isBotProduct() ? 'Orgo workspace is unreachable' : remoteReauth ? copy.remoteTitle : copy.title}
            </h2>
            <p className="mt-1 text-[0.8125rem] leading-5 text-(--ui-text-tertiary)">
              {isBotProduct() ? 'The Mac app could not connect. Your remote conversations and files remain on Orgo.' : remoteReauth ? copy.remoteDescription : copy.description}
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 p-5 pt-0">
          ...
          {logs.length > 0 ? (
            <div className="grid gap-2">
              ...
              {isBotProduct() ? <ComputerWindows /> : null}
            </div>
          ) : null}
        </div>
      </div>
