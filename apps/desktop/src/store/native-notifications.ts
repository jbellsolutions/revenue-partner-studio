...
export async function respondToApprovalAction(sessionId: null | string, actionId: string): Promise<void> {
  const [action, requestId] = actionId.split(':')
  ...
  try {
    const result = await gateway.request<{ resolved?: boolean | number }>('approval.respond', {
      choice, session_id: sessionId ?? undefined, ...(requestId ? { request_id: requestId } : {})
    })
    if (result.resolved) clearApprovalRequest(sessionId, requestId)
  } catch {
