...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  if (!allowsGenericHermesUpdates()) {
    return {
      ok: false,
      error: 'unavailable',
      message: 'Updates are checked daily and installed by verified Orgo AI Guy Bot maintenance.'
    }
  }
...
export function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  if (!allowsGenericHermesUpdates()) {
    return Promise.resolve({
      ok: false,
      error: 'unavailable',
      message: 'Backend updates are managed by verified Orgo AI Guy Bot maintenance.'
    })
  }
