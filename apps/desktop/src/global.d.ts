...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      orgoDesktop: {
        previewHistory: (payload: { profile: string; sessionId: string }) => Promise<{ title: string; text: string; truncated: boolean }>
        historyTargets: (computerId: string) => Promise<{ profiles: string[] }>
        shareHistory: (payload: { sourceProfile: string; sourceSessionId: string; targetComputerId: string; targetProfile: string; text: string; requestId: string }) => Promise<{ state: string; requestId: string; sessionId?: string; error?: string }>
        listInstances: (refresh?: boolean) => Promise<Array<{ name: string; computerId: string; current: boolean; unreadable: boolean; isDefault?: boolean; cloudName?: string; cloudStatus?: string; availability?: 'available' | 'missing' | 'unknown' }>>
        openComputer: (payload: {computerId:string;apiKey?:string}) => Promise<{opened:boolean;name:string;reused:boolean;mode:'studio'|'hermes'|'desktop'}>
        setDefaultInstance: (name: string) => Promise<{instance: string}>
        fitWindow: () => Promise<void>
        openInstance: (name: string) => Promise<{ opened: boolean }>
        ...
        setScreenPaused: (profile: string, paused: boolean) => Promise<{ paused: boolean }>
        ...
        syncSkills: (sourcePath?: string) => Promise<{
          cancelled: boolean
          syncedNow: boolean
          sourcePath: string
          skillCount: number
...
export interface DesktopConnectionConfig {
  ...
  // Whether protected desktop secret storage is currently available on this
  // machine. When false, a persisted remote token can only be stored as plain
  // text on disk (with an explicit opt-in).
...
export interface DesktopConnectionConfigInput {
  ...
  remoteAuthMode?: 'oauth' | 'token'
  remoteToken?: string
  // When true and protected secret storage is unavailable, persist the
