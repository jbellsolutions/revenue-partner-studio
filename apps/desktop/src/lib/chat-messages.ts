...
import type { MessageReaction, SessionMessage, TaskUpdate, UsageStats } from '@/types/hermes'
...
export type ChatMessage = {
  ...
  /** Explicit backend task deliveries; an empty list retains legacy notice text. */
...
function taskUpdates(metadata: SessionMessage['display_metadata']): TaskUpdate[] {
  const rows = parseDisplayMetadata(metadata)?.task_updates
  ...
  return rows.flatMap(row => {
    ...
    const update: TaskUpdate = { task_id: row.task_id, board: row.board, text: row.text }

    for (const key of ['title', 'worker', 'worker_session_id', 'assignee', 'kind'] as const) {
...
function taskDeliveryId(message: SessionMessage): string | undefined {
  if (message.display_kind !== 'kanban_notification') {
    return undefined
  }
  const ids = parseDisplayMetadata(message.display_metadata)?.delivery_ids

  return Array.isArray(ids) && ids.length > 0 && ids.every(id => typeof id === 'string' && id)
    ? `task-delivery:${JSON.stringify(ids)}`
    : undefined
}
...
export function toChatMessages(messages: SessionMessage[]): ChatMessage[] {
  ...
  messages.forEach((message, index) => {
    ...
    const displayRole =
      ...
      message.display_kind === 'kanban_notification' ||
    ...
    if (message.role === 'assistant') {
      ...
      // A persisted assistant iteration with visible prose is a real
      // conversational update (the same message.interim bubble shown live),
      // even when it also launched the next tool. Keep it as a new message so
      // reload does not collapse "On it…" + later updates + final into one
      // oversized bubble. Tool/reasoning-only carriers can still fold into the
      // active message for correlation and technical-mode continuity.
      if (activeAssistant && !displayContent && (currentHasToolCall || activeHasToolCall)) {
    ...
    result.push({
      id: taskDeliveryId(message) ?? `${message.timestamp || Date.now()}-${index}-${displayRole}`,
      role: displayRole,
      parts,
      timestamp: message.timestamp,
      ...(message.display_kind === 'kanban_notification' ? { taskUpdates: taskUpdates(message.display_metadata) } : {}),
