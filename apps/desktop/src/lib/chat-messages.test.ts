...
describe('toChatMessages', () => {
  ...
  it('keeps an interleaved tool-only row with the update that launched it', () => {
    const messages = toChatMessages([
    ...
    expect(messages[0].parts.map(p => p.type)).toEqual(['text', 'tool-call'])
    expect(messages.map(chatMessageText)).toEqual(['Planning.', 'Done.'])
  })

  it('rehydrates assistant tool-call iterations as separate conversational bubbles', () => {
    const messages = toChatMessages([
    ...
    expect(assistantMessages.flatMap(message => message.parts).filter(part => part.type === 'tool-call')).toHaveLength(
      2
    )
    expect(assistantMessages.map(chatMessageText)).toEqual([
      "Let me also check if there's a top-level lint workflow.",
      'No CI in this repo. Build is enough.',
...
describe('renderMediaTags', () => {
  it('keeps task deliveries distinct from human messages with typed attribution', () => {
    const [notice, human] = toChatMessages([
      {
        role: 'user',
        ...
        display_metadata: JSON.stringify({
          delivery_ids: ['d1'],
          task_updates: [
            { task_id: 't1', board: 'team', worker: 'researcher', kind: 'completed', text: 'verified result' }
          ]
        }),
        timestamp: 1
      },
      { role: 'user', content: 'Message from researcher: verified result', timestamp: 2 }
    ])

    expect(notice.role).toBe('system')
    expect(notice.taskUpdates?.[0]).toMatchObject({ task_id: 't1', worker: 'researcher' })
    expect(human.role).toBe('user')
    expect(human.taskUpdates).toBeUndefined()
  })

  it.each(['{bad', 'null', '[]', JSON.stringify({ task_updates: [null, 3, { worker: 'fake' }] })])(
    'preserves task notification text with malformed or old metadata %s',
    metadata => {
      const [notice] = toChatMessages([
        { role: 'user', content: 'saved result', display_kind: 'kanban_notification', display_metadata: metadata }
      ])

      expect(notice.role).toBe('system')
      expect(chatMessageText(notice)).toBe('saved result')
      expect(notice.taskUpdates).toEqual([])
    }
  )
