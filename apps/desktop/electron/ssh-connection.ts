...
function defaultControlDir() {
  ...
  return path.join(os.homedir(), '.hermes-orgo-studio', 'ssh')
}
...
const TAILSCALE_SSH_CHECK_URL_RE = /https:\/\/login\.tailscale\.com\/a\/[A-Za-z0-9_-]+/

// Tailscale SSH's `check` action writes a one-time browser URL before the SSH
// process waits. Treat only the exact HTTPS origin + action path as trusted;
// this URL is later offered to the renderer as a user-initiated recovery link.
function extractTailscaleSshCheckUrl(value) {
  const match = String(value || '').match(TAILSCALE_SSH_CHECK_URL_RE)
...
function classifySshError(stderr) {
  ...
  if (extractTailscaleSshCheckUrl(text)) {
    return SSH_ERROR.REAUTH_REQUIRED
  }
...
function sshErrorMessage(kind, conn, stderr?) {
  ...
  switch (kind) {
    case SSH_ERROR.REAUTH_REQUIRED: {
      const authUrl = extractTailscaleSshCheckUrl(stderr)

      return (
        `Tailscale needs a quick identity check before SSH can connect to ${host}. ` +
        `Complete it in your browser${authUrl ? `: ${authUrl}` : '.'}`
      )
    }
...
function runSsh(args, { timeoutMs, spawnFn = spawn, stdin = 'ignore', stdinData }: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      // SSH can emit useful diagnostics before it hangs. Tailscale SSH in
      // particular prints a browser authorization URL and then waits for the
      // user, so retain partial output for the classifier instead of replacing
      // it with a generic timeout.
      err.stdout = stdout
      err.stderr = stderr
      reject(err)
    }, timeoutMs)
  ...
  _fail(stderrOrErr, fallbackKind = SSH_ERROR.UNKNOWN) {
    const detail =
      typeof stderrOrErr === 'string'
        ? stderrOrErr
        : [stderrOrErr?.stderr, stderrOrErr?.stdout, stderrOrErr?.message].filter(Boolean).join('\n')

    const classified = detail ? classifySshError(detail) : SSH_ERROR.UNKNOWN

    const kind =
      classified !== SSH_ERROR.UNKNOWN
        ? classified
        : stderrOrErr?.kind === SSH_ERROR.TIMEOUT
          ? SSH_ERROR.TIMEOUT
          : fallbackKind

    const err: any = new Error(sshErrorMessage(kind, this, detail))
    err.kind = kind

    if (kind === SSH_ERROR.REAUTH_REQUIRED) {
      err.tailscaleAuthUrl = extractTailscaleSshCheckUrl(detail)
    }
