import { connectExistingHermes } from './hermes-existing-connection'
...
import { inspectStudioComputer, prepareStudioComputer, readStudioComputer, readStudioWindow } from './studio-computers'
...
import { botInstanceLaunch, listBotInstances, refreshBotInstances } from './bot-instances'
import {
...
import { type HistoryHandoff, type HistoryHandoffResult, type HistoryPreview, readCollaborationAssets, runCollaboration } from './orgo-collaboration'
import {
...
import { legacyScreenSession, workspaceScreenSession, screenCommand } from './orgo-screen-session'
...
let BOT_INSTANCE_NAME = ''

try {
  BOT_INSTANCE_NAME = isBotProduct() && !USER_DATA_OVERRIDE
    ? resolveStudioLaunch(path.join(app.getPath('appData'), 'Hermes Orgo Studio'), process.argv)
    : currentBotInstanceName(process.argv)
} catch (error: any) {
  dialog.showErrorBox('Studio could not open the selected computer', error?.message || String(error))
  app.exit(2)
  throw error
}
...
applyDesktopProductIdentity(app, BOT_INSTANCE_NAME)
...
const USE_BOT_LOCAL_SECRET_VAULT = shouldUseBotLocalSecretVault({
  platform: process.platform,
  botProduct: isBotProduct()
})
...
// A source-built macOS app is ad-hoc signed, so every rebuild has a different
// Keychain identity. Chromium otherwise keeps asking to access the previous
// build's "Safe Storage" item before our recovery UI can appear. The mock
// backend prevents Chromium cookie storage from touching Keychain; secrets the
// app owns use the authenticated local vault below instead.
if (USE_BOT_LOCAL_SECRET_VAULT) {
...
function resolveHermesHome() {
  if (isBotProduct()) {
    // Mac-side connection metadata only; never adopt an inherited agent home.
    return path.join(app.getPath('userData'), 'connection-home')
  }

  if (BOT_INSTANCE_NAME) {
    return path.join(app.getPath('home'), '.hermes-korgo', 'instances', BOT_INSTANCE_NAME)
  }
  ...
  if (isBotProduct()) {
    const botHome = path.join(app.getPath('home'), '.hermes-korgo')

    return BOT_INSTANCE_NAME ? path.join(botHome, 'instances', BOT_INSTANCE_NAME) : botHome
  }
...
if (BOT_INSTANCE_NAME) {
  console.log(
    `[hermes] Orgo instance '${BOT_INSTANCE_NAME}' uses userData=${app.getPath('userData')} HERMES_HOME=${HERMES_HOME}`
  )
}
...
configureStudioBinding(() => {
  try { return JSON.parse(fs.readFileSync(DESKTOP_ORGO_CONFIG_PATH, 'utf8')).profiles?.default?.computerId || '' }
  catch { return '' }
})
const DESKTOP_LOCAL_SECRET_KEY_PATH = path.join(app.getPath('userData'), 'local-secret.key')
const ORGO_CLI_CREDENTIALS_PATH = path.join(app.getPath('userData'), 'orgo-credentials.json')
...
const APP_NAME = desktopAppName(BOT_INSTANCE_NAME)
...
const APP_ICON_PNG = isBotProduct() ? 'studio-icon.png' : 'apple-touch-icon.png'
const APP_ICON_ICO = isBotProduct() ? 'studio-icon.ico' : 'icon.ico'
...
function buildApplicationMenu() {
  ...
  template.push({
    ...
    submenu: [
      ...
      { click: () => createInstanceWindow(), label: isBotProduct() ? 'Show This Computer' : 'New Window' },
...
// involved. Tokens are persisted through the selected desktop secret store,
...
// The electron-coupled half of the token store: secret encryption plus the
...
function encryptDesktopSecret(value, options = {}) {
  if (USE_BOT_LOCAL_SECRET_VAULT) {
    return encryptLocalSecret(value, DESKTOP_LOCAL_SECRET_KEY_PATH)
  }
...
function decryptDesktopSecret(secret) {
  ...
  if (secret.encoding === LOCAL_SECRET_VAULT_ENCODING) {
    try {
      return decryptLocalSecret(secret, DESKTOP_LOCAL_SECRET_KEY_PATH)
    } catch {
      return ''
    }
  }
  ...
  if (secret.encoding === SAFE_STORAGE_ENCODING) {
    // Never ask Keychain to unlock a blob owned by an earlier ad-hoc build.
    // Known Orgo credentials migrate from the official CLI file below; other
    // unavailable legacy secrets are re-entered only when their feature is used.
    if (USE_BOT_LOCAL_SECRET_VAULT) {
      return ''
    }
    ...
    try {
      ...
      // Never shell out to `/usr/bin/security` to export an old app identity's
      // Safe Storage password. macOS correctly prompts for that cross-process
      // read on every launch. A renamed app asks for the Orgo key once and
      // encrypts it under its final identity instead.
      return ''
    }
  }
...
function writeDesktopConnectionConfig(config) {
  ...
  // encrypted gateway token plus its URL and SSH host/user/keyPath. Mode 0600
  // keeps the whole record — including fields that are not encrypted — off
  // other local accounts, matching native-oauth-tokens.json and
...
// Profiles may still save an explicit override. The API key is encrypted by the
// selected desktop secret store; the renderer only receives whether a key is
// present, never the key itself. VNC credentials are intentionally not persisted
// because Orgo rotates them when a computer restarts.
  ...
  try {
    ...
    for (const [rawProfile, rawEntry] of Object.entries(parsed.profiles)) {
      ...
      const supportedEncoding =
        apiKey?.encoding === SAFE_STORAGE_ENCODING ||
        (USE_BOT_LOCAL_SECRET_VAULT && apiKey?.encoding === LOCAL_SECRET_VAULT_ENCODING)

      if (!supportedEncoding || typeof apiKey.value !== 'string' || !apiKey.value) {
        continue
      }
...
function readOrgoCliApiKey() {
  try {
    return parseOrgoCliApiKey(fs.readFileSync(ORGO_CLI_CREDENTIALS_PATH, 'utf8'))
  } catch {
    return ''
  }
}

function decryptOrgoApiKey(secret) {
  // An explicitly saved instance credential owns its binding. CLI credentials
  // only bootstrap an unset key or recover a legacy Keychain-backed value.
  if (secret?.encoding === LOCAL_SECRET_VAULT_ENCODING) {
    return decryptDesktopSecret(secret)
  }

  return readOrgoCliApiKey() || decryptDesktopSecret(secret)
}

function decryptSshReuseToken(secret) {
  return decryptDesktopSecret(secret)
}

function encryptSshReuseToken(token) {
  return encryptDesktopSecret(token)
}

function migrateBotOrgoSecretFromCli() {
  if (!USE_BOT_LOCAL_SECRET_VAULT) {
    return false
  }

  try {
    const apiKey = readOrgoCliApiKey()
    ...
    const current = config.profiles?.default

    if (!current || current.apiKey?.encoding !== SAFE_STORAGE_ENCODING) {
      return false
    }

    config.profiles.default = { ...current, apiKey: encryptDesktopSecret(apiKey) }
    writeOrgoDesktopConfig(config)
    rememberLog('[security] migrated the Orgo credential from legacy Keychain storage to the local vault')

    return true
  } catch (error: any) {
    rememberLog(`[security] could not migrate the Orgo credential to the local vault: ${error.message}`)
...
function orgoBackendEnv() {
  try {
    ...
    const apiKey = decryptOrgoApiKey(entry?.apiKey)
...
async function upsertOrgoMcp(profile, computerId) {
  ...
  const existing: any = await composioProfileApi(profile, { path: '/api/config', method: 'GET' })
  // A profile sync must not replace the explicitly installed subscription
  // worker/credential resolver with the older global-runtime launch commands.
  for (const [name, executable] of [['orgo-agent', 'worker'], ['orgo', 'native']]) {
    const current = existing?.mcp_servers?.[name]
    if (current?.command === `/root/.hermes/orgo-computer/${executable}` &&
        current?.env?.ORGO_DEFAULT_COMPUTER_ID === computerId) {
  ...
  const servers = Object.fromEntries(
    Object.entries(entries).map(([name, entry]) => [
      name,
      {
        ...
        env: entry.env,
        trust: entry.trust,
        ...('enabled' in entry ? { enabled: entry.enabled } : {}),
...
async function syncOrgoMcpToProfiles(profileNames) {
  if (isBotProduct()) return { synced: 0, computerId: defaultOrgoDesktopCredentials().computerId }
...
function sanitizeOrgoDesktopConfig(profile) {
  ...
  const apiKeySet = Boolean(decryptOrgoApiKey(entry?.apiKey))

  return {
    configured: Boolean(entry?.computerId && apiKeySet),
...
function saveOrgoDesktopConfig(payload) {
  ...
  const computerId = enforceOrgoComputerBinding(current?.computerId, payload?.computerId)
...
async function provisionOrgoSharedComputer() {
  throw new Error('Automatic provisioning is disabled for this trial. The dedicated Studio computer must be bound explicitly.')

  const current = resolveOrgoDesktopProfile(readOrgoDesktopConfig().profiles, 'default').entry
  const apiKey = decryptOrgoApiKey(current?.apiKey)
  ...
  if (!workspaceId && !computerId) {
    const workspaceName = isBotProduct()
      ? BOT_ORGO_WORKSPACE_NAME + (BOT_INSTANCE_NAME ? ' — ' + BOT_INSTANCE_NAME : '')
      : 'Hermes'

    const workspace = await findOrCreateOrgoWorkspace(
      apiKey,
      workspaceName,
      isBotProduct() && !BOT_INSTANCE_NAME ? [...BOT_ORGO_LEGACY_WORKSPACE_NAMES] : []
    )
  ...
  if (!isBotProduct()) await persistOrgoEnvironmentOnRemote(apiKey, computerId)
  await ensureOrgoMaintenanceInstalled(apiKey, computerId, () =>
    readBundledOrgoMaintenanceAssets(APP_ROOT, process.resourcesPath)
  )
...
function defaultOrgoDesktopCredentials() {
  const config = readOrgoDesktopConfig()
  const entry = resolveOrgoDesktopProfile(config.profiles, 'default').entry
  const apiKey = decryptOrgoApiKey(entry?.apiKey)
...
async function importLocalKorgoSkills(requestedSourcePath?: string) {
  const credentials = defaultOrgoDesktopCredentials()

  const candidates = [
    path.join(process.env.HERMES_HOME || path.join(os.homedir(), '.hermes-korgo'), 'skills'),
    path.join(os.homedir(), 'Desktop', 'Mac Main', 'Hermes', 'Skills'),
    path.join(os.homedir(), '.hermes', 'skills')
  ].map(candidate => path.resolve(candidate))

  const defaultPath = candidates.find(candidate => fs.existsSync(candidate)) || os.homedir()
  let sourcePath = ''

  if (requestedSourcePath) {
    sourcePath = path.resolve(requestedSourcePath)

    if (!candidates.includes(sourcePath)) {
      throw new Error('Skills sync source must be the local Hermes, Korgo, or Obsidian skills directory.')
    }
  } else {
    const selected = await dialog.showOpenDialog({
      title: 'Import Hermes skills to the Korgo computer',
    ...
    if (selected.canceled || !selected.filePaths[0]) {
      return { cancelled: true, syncedNow: false, sourcePath: '', skillCount: 0, fileCount: 0 }
    }

    sourcePath = path.resolve(selected.filePaths[0])
  }

  const bundle = buildKorgoSkillsBundle(sourcePath)
  const manifest = JSON.parse(zlib.gunzipSync(bundle).toString('utf8')) as { files?: Record<string, string> }
  const paths = Object.keys(manifest.files || {})
  const skillCount = paths.filter(name => path.posix.basename(name) === 'SKILL.md').length

  await ensureOrgoComputerRunning(credentials.apiKey, credentials.computerId)
  const result = await ensureKorgoSkillsOnOrgo(credentials.apiKey, credentials.computerId, () => bundle)
...
async function connectHermesThroughOrgoTailscale() {
  ...
  const connection = await applyDesktopConnectionConfig({
    ...
    sshPort: 22,
    sshKeyPath: '',
    sshRemoteHermesPath: hermes.hermesPath || '',
    sshRemoteProfile: ''
  })
...
async function createOrgoDesktopSession(profile) {
  ...
  try {
    ...
    const session = await fetchOrgoDesktopSession({ apiKey, computerId: entry.computerId })
    // A desktop-only window must never start or attach an agent backend.
    if (isBotProduct() && readStudioWindow(app.getPath('userData')).mode === 'desktop') return {ok: true, ...session}
    let target = activeSshTerminalTarget(key)
    if (target === 'pending') {
      await ensureBackend(key)
      target = activeSshTerminalTarget(key)
    }
    if (typeof target === 'string') {
      throw new Error('The computer connection is reconnecting. Retry shortly.')
    }
    const screenWorkspace = isBotProduct() && readStudioWindow(app.getPath('userData')).mode === 'hermes' ? 'hermes' : 'studio'
    const managed = target ? await workspaceScreenSession(target.ssh, entry.computerId, key, session.password, async () => Number(await pickLocalPort()), screenWorkspace) : null
    if (isBotProduct() && readStudioWindow(app.getPath('userData')).mode==='studio' && !managed) throw new Error('The selected agent screen is not connected yet.')
    // Only explicitly unsupported managed screens use the legacy main desktop.
    // Errors/wrong identities above fail closed and never change the target.
    const legacy = !managed && target
      ? await legacyScreenSession(target.ssh, entry.computerId, session.password, async () => Number(await pickLocalPort()))
      : null
    return { ok: true, ...session, ...legacy, ...managed }
  } catch (error) {
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  let secureTokenStorage = USE_BOT_LOCAL_SECRET_VAULT
...
function activeSshTerminalTarget(profile = primaryProfileKey()) {
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    if (isBotProduct()) {
      const connect = readStudioWindow(app.getPath('userData')).mode === 'hermes' ? connectExistingHermes : connectStudioService
      result = await connect({ computerId: defaultOrgoDesktopCredentials().computerId, ssh, pickLocalPort: async () => Number(await pickLocalPort()),
...
function persistSshConnectionToken(profile, source, token) {
  try {
    const config = readDesktopConnectionConfig()
    const encrypted = encryptSshReuseToken(token)
...
async function resolveRemoteBackend(profile) {
  ...
  if (sshOverride) {
    const reuseToken = decryptSshReuseToken(config.profiles?.[connectionScopeKey(profile)]?.token)
  ...
  if (config.mode === 'ssh') {
    ...
    const reuseToken = decryptSshReuseToken(config.remote?.token)
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (isBotProduct()) throw new Error('Studio requires a remote Hermes connection for every agent.')
...
async function startHermes() {
  if(isBotProduct() && readStudioWindow(app.getPath('userData')).mode==='desktop') throw new Error('This window connects to the existing computer only; its agent runtime is managed by AI Guy.')
  ...
  const connectionPromise = (async () => {
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        if (isBotProduct()) throw new Error('Studio agents run on the dedicated Orgo computer. Configure the remote connection to continue.')
    ...
    updateBootProgress(
      {
        error: message,
        message: `${attemptedRemote ? 'Remote workspace connection failed' : 'Desktop boot failed'}: ${message}`,
        phase: 'backend.error',
        running: false
      },
      { allowDecrease: true }
...
function createInstanceWindow() {
  // Studio owns one main window per computer. A generic peer loses the
  // desktop-only route and incorrectly boots the Head of Operations UI.
  if (isBotProduct()) {
    ensureMainWindow(mainWindow, {isReady: app.isReady(), createWindow, focusWindow, focusExisting: true})
    return mainWindow
  }
  const icon = getAppIconPath()
...
function createWindow() {
  const icon = getAppIconPath()
  const savedWindowState = isBotProduct() ? studioRestoredWindowState(readWindowState(), screen.getAllDisplays()) : readWindowState()
  ...
  const computerOnly=isBotProduct() && readStudioWindow(app.getPath('userData')).mode==='desktop'
  const mainUrl=new URL(DEV_SERVER || pathToFileURL(resolveRendererIndex()).toString())
  if(computerOnly) mainUrl.searchParams.set('win','studio-computer')
  if(isBotProduct() && readStudioWindow(app.getPath('userData')).mode==='hermes') mainUrl.searchParams.set('workspace','hermes')
  loadWindowUrl(mainWindow, mainUrl.toString(), 'Renderer')
  ...
  if (!computerOnly) startHermes().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('hermes:orgo-desktop:control', async (_event, profile, paused) => {
  if (typeof paused !== 'boolean') {
    throw new Error('Invalid screen control request.')
  }
  const key = orgoDesktopProfileKey(profile)
  const entry = resolveOrgoDesktopProfile(readOrgoDesktopConfig().profiles, key).entry
  const target = activeSshTerminalTarget(key)
  if (!entry?.computerId || !target || target === 'pending') {
    throw new Error('Connect the selected cloud computer first.')
  }
  const result = await screenCommand(target.ssh, entry.computerId, key, paused ? 'pause' : 'resume')
  if (!result) {
    throw new Error('This computer does not support managed screen control yet.')
  }
...
function knownCollaborationComputer(computerId?: string) {
  if (!isBotProduct() || process.env.HERMES_DESKTOP_USER_DATA_DIR) {throw new Error('Handoffs require a configured Orgo computer window.')}
  const source = defaultOrgoDesktopCredentials()
  const target = computerId || source.computerId
  const instances = listBotInstances(path.join(app.getPath('appData'), 'Hermes Orgo Studio'), BOT_INSTANCE_NAME)

  if (target !== source.computerId && !instances.some(instance => !instance.unreadable && instance.computerId === target)) {
    throw new Error('Open and configure the destination computer window before sharing history.')
  }

  return { ...source, target }
}

ipcMain.handle('hermes:orgo-desktop:history:preview', async (_event, payload: { profile: string; sessionId: string }) => {
  const source = knownCollaborationComputer()

  return runCollaboration<HistoryPreview>(source.apiKey, source.computerId, readCollaborationAssets(APP_ROOT, process.resourcesPath), 'preview', payload)
})
ipcMain.handle('hermes:orgo-desktop:history:targets', async (_event, computerId: string) => {
  const source = knownCollaborationComputer(computerId)

  return runCollaboration<{ profiles: string[] }>(source.apiKey, source.target, readCollaborationAssets(APP_ROOT, process.resourcesPath), 'profiles', {})
})
ipcMain.handle('hermes:orgo-desktop:history:share', async (_event, payload: HistoryHandoff) => {
  const source = knownCollaborationComputer(payload.targetComputerId)

  if (!/^[a-z0-9][a-z0-9_-]{0,63}$/.test(payload.sourceProfile)) {throw new Error('Invalid source profile.')}

  return runCollaboration<HistoryHandoffResult>(source.apiKey, source.target, readCollaborationAssets(APP_ROOT, process.resourcesPath), 'deliver', {
    targetProfile: payload.targetProfile, text: payload.text, requestId: payload.requestId, sourceSessionId: payload.sourceSessionId,
    sender: `${source.computerId}/${payload.sourceProfile}`
  })
})
ipcMain.handle('hermes:orgo-desktop:instances', async (_event, refresh?: boolean) => {
  if (!isBotProduct()) {throw new Error('Computer windows are available in Revenue Partner Studio.')}

  if (process.env.HERMES_DESKTOP_USER_DATA_DIR) {throw new Error('Computer switching is unavailable in an isolated test window.')}

  const root = path.join(app.getPath('appData'), 'Hermes Orgo Studio')
  const preferred = readStudioDefault(root) ?? ''
  const instances = listBotInstances(root, BOT_INSTANCE_NAME).map(row => ({...row, isDefault: row.name === preferred}))
  ...
  return refreshBotInstances(instances, computerId => {
    if (!apiKey) {
      return Promise.reject(new Error('No current-account credential'))
    }

    return readStudioComputer(apiKey, computerId, (url, init) =>
      fetch(url, { ...init, signal: AbortSignal.timeout(8000) })
    )
  })
})
async function openStudioInstance(name: unknown) {
  if (!isBotProduct() || !app.isPackaged || process.env.HERMES_DESKTOP_USER_DATA_DIR) {
    throw new Error('Computer switching requires the installed Revenue Partner Studio app.')
  }

  const launch = botInstanceLaunch(name, process.execPath, process.platform, process.env)

  if (process.platform === 'darwin') {
    await new Promise<void>((resolve, reject) => execFile(launch.command, launch.args, { env: launch.env }, error => error ? reject(error) : resolve()))
  } else {
    await new Promise<void>((resolve, reject) => {
      const child = spawn(launch.command, launch.args, { env: launch.env, detached: true, stdio: 'ignore' })
      child.once('error', reject)
...
ipcMain.handle('hermes:orgo-desktop:instance:open', async (_event, name: unknown) => openStudioInstance(name))
ipcMain.handle('hermes:orgo-desktop:instance:default', async (_event, name: unknown) => {
  if (!isBotProduct() || USER_DATA_OVERRIDE) throw new Error('Use the installed Studio workspace.')
  return {instance: saveStudioDefault(path.join(app.getPath('appData'), 'Hermes Orgo Studio'), name)}
})
ipcMain.handle('hermes:orgo-desktop:window:fit', async event => {
  const win = BrowserWindow.fromWebContents(event.sender)
  if (!isBotProduct() || !win || win.isDestroyed()) return
  if (win.isFullScreen()) {
    await new Promise<void>(resolve => {
      const finish = () => {clearTimeout(timer); win.removeListener('leave-full-screen', finish); resolve()}
      const timer = setTimeout(finish, 3000)
      win.once('leave-full-screen', finish); win.setFullScreen(false)
    })
    if (win.isDestroyed()) return
    if (win.isFullScreen()) throw new Error('The Mac is still leaving fullscreen. Try Fit window again.')
  }
  win.unmaximize()
  const display = screen.getDisplayMatching(win.getBounds())
  win.setBounds(computeWindowOptions(null, [display]))
  win.center()
  persistWindowState()
})
ipcMain.handle('hermes:orgo-desktop:computer:open', async (_event, payload: {computerId:string;apiKey?:string}) => {
  if (!isBotProduct() || !app.isPackaged || process.env.HERMES_DESKTOP_USER_DATA_DIR) throw new Error('Use the installed Studio app to open another computer.')
  const current=resolveOrgoDesktopProfile(readOrgoDesktopConfig().profiles,'default').entry
  const apiKey=String(payload.apiKey||decryptOrgoApiKey(current?.apiKey)||'').trim()
  if (!apiKey) throw new Error('Enter an Orgo API key.')
  const computer=await readStudioComputer(apiKey,payload.computerId)
  const connection=await inspectStudioComputer(apiKey,computer.id)
  if(connection.mode==='desktop') throw new Error('No compatible Hermes chat workspace was found on that computer. Your current workspace has not changed.')
  const result=prepareStudioComputer(path.join(app.getPath('appData'),'Hermes Orgo Studio'),computer,apiKey,connection)
  await openStudioInstance(result.name)
...
ipcMain.handle('hermes:orgo-desktop:skills:sync', async (_event, sourcePath?: string) =>
  importLocalKorgoSkills(sourcePath)
)
ipcMain.handle('hermes:orgo-desktop:computer:get', async () => {
  const credentials = defaultOrgoDesktopCredentials()

  return getOrgoComputer(credentials.apiKey, credentials.computerId)
})
...
ipcMain.handle('hermes:updates:check', async () =>
  !allowsGenericHermesUpdates()
    ? {
        supported: false,
        message: 'Updates are checked daily by Revenue Partner Studio release maintenance.',
        fetchedAt: Date.now()
      }
...
ipcMain.handle('hermes:updates:apply', async (_event, payload) =>
  !allowsGenericHermesUpdates()
    ? {
        ok: false,
        error: 'unavailable',
        message: 'Run the Revenue Partner Studio release maintenance updater.'
      }
...
// ---------------------------------------------------------------------------
const HERMES_PROTOCOL = 'hermes-orgo-studio'
...
function registerDeepLinkProtocol() {
  // hermes:// links do not identify an Orgo instance. Keep their owner the
  // legacy/default instance instead of letting the last named instance steal
  // every link on the machine.
  if (BOT_INSTANCE_NAME) {
    return
  }
...
// Chromium scopes this lock to Electron's userData directory. Named Orgo
// instances set distinct userData roots before requesting it, so each name can
// run once while a duplicate launch focuses only that same instance.
const _gotSingleInstanceLock = app.requestSingleInstanceLock({ orgoInstance: BOT_INSTANCE_NAME || 'default' })
...
function heldQuitForActiveWork(event: Electron.Event): boolean {
  if (isBotProduct() || SKIP_QUIT_CONFIRM || quitConfirmedWithActiveWork || quitPromptOpen) {
    return false
  }
