...
test('bootstrap installer URL never anonymously targets the private product repository', () => {
  assert.throws(
    () => bootstrapInstallScriptUrl('a'.repeat(40), { botProduct: true }),
    /signed packaged installer/
  )
  assert.equal(
    bootstrapInstallScriptUrl('main', { botProduct: false }),
    `https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/${SCRIPT_NAME}`
  )
})

test('bot bootstrap overrides the installer clone repository', () => {
  assert.deepEqual(bootstrapRepositoryEnvironment(true), {
    HERMES_INSTALL_REPO_URL_HTTPS: 'https://github.com/jbellsolutions/orgo-ai-guy-bot.git',
    HERMES_INSTALL_REPO_URL_SSH: 'git@github.com:jbellsolutions/orgo-ai-guy-bot.git'
  })
  assert.deepEqual(bootstrapRepositoryEnvironment(false), {})
})
...
test('fresh bot bootstrap uses the signed packaged installer without network access', async () => {
  const home = mkTmpHome()
  const resources = mkTmpHome()

  try {
    const packagedDir = path.join(resources, 'bootstrap')
    fs.mkdirSync(packagedDir, { recursive: true })
    const packaged = path.join(packagedDir, SCRIPT_NAME)
    fs.writeFileSync(packaged, '#!/bin/sh\necho packaged\n')
    let downloadCalls = 0

    assert.equal(packagedInstallScript(resources), packaged)
...
test('fresh bot bootstrap fails closed when the signed packaged installer is missing', async () => {
  const home = mkTmpHome()
  const resources = mkTmpHome()
  let downloadCalls = 0
  ...
  try {
    await assert.rejects(
      ...
      /signed packaged installer is missing/
    )
    assert.equal(downloadCalls, 0)
  } finally {
    fs.rmSync(home, { recursive: true, force: true })
