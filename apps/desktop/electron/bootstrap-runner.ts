...
import { BOT_SOURCE_REPO_URL, isBotProduct } from './product'
...
function packagedInstallScript(resourcesRoot = process.resourcesPath) {
  if (!resourcesRoot) {
    return null
  }

  const candidate = path.join(resourcesRoot, 'bootstrap', installScriptName())
...
function bootstrapInstallScriptUrl(ref, opts: { botProduct?: boolean } = {}) {
  const botProduct = opts.botProduct ?? isBotProduct()

  if (botProduct) {
    throw new Error('Revenue Partner Studio must use its signed packaged installer, not an anonymous private-repository URL')
  }

  const scriptName = installScriptName()

  return `https://raw.githubusercontent.com/NousResearch/hermes-agent/${ref}/scripts/${scriptName}`
}

function bootstrapRepositoryEnvironment(botProduct = isBotProduct()) {
  if (!botProduct) {
    return {}
  }

  return {
    HERMES_INSTALL_REPO_URL_HTTPS: BOT_SOURCE_REPO_URL,
    HERMES_INSTALL_REPO_URL_SSH: 'git@github.com:jbellsolutions/orgo-ai-guy-bot.git'
  }
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const scriptName = installScriptName()
  const url = bootstrapInstallScriptUrl(ref)
...
async function resolveInstallScript({
  ...
  const packaged = packagedInstallScript(resourcesRoot)

  if (packaged) {
    emit({ type: 'log', line: `[bootstrap] using signed packaged ${installScriptName()} at ${packaged}` })

    return { path: packaged, source: 'packaged', kind: installScriptKind() }
  }

  if (botProduct) {
    throw new Error(`Cannot bootstrap Revenue Partner Studio: signed packaged installer is missing (${installScriptName()})`)
  }

  // Generic Hermes packages may retrieve their public installer by immutable ref.
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          ...bootstrapRepositoryEnvironment(),
