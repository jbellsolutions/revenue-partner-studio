...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  ...
  assert.match(cmd, /ORGO_API_KEY=\*/)
  assert.match(cmd, /export.*\$line/)
  assert.doesNotMatch(cmd, /eval/)
})

test('probeHermesVersion imports the Orgo env and ignores config warnings before the version', async () => {
  const ssh = fakeSsh([
    [
      /--version/,
      "Config ref '${env:ORGO_API_KEY}' is not set\nHermes Agent v0.21.0 (2026.8.31)\nInstall directory: /x"
    ]
  ])

  assert.equal(await probeHermesVersion(ssh, '/x/hermes'), 'Hermes Agent v0.21.0 (2026.8.31)')
  assert.match(ssh.calls[0] || '', /ORGO_DEFAULT_COMPUTER_ID=\*/)
})
...
test('buildSpawnCommand raises the SSH child file limit before execing Hermes', () => {
  const cmd = buildSpawnCommand('/x/hermes', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true;/)
  assert.ok(cmd.indexOf('ulimit -n 65536') < cmd.indexOf('exec env HERMES_DESKTOP=1'))
