...
// Hermes currently resolves config env references before it loads this file on
// some CLI paths. Import only the two Orgo values written by our own broker,
// using `export "$line"` (data, never eval) so metacharacters stay literal.
const REMOTE_ORGO_ENV_LOADER =
  'if [ -r "$HOME/.hermes/.env" ]; then while IFS= read -r line; do ' +
  'case "$line" in ORGO_API_KEY=*|ORGO_DEFAULT_COMPUTER_ID=*) export "$line" ;; esac; ' +
  'done < "$HOME/.hermes/.env"; fi; '
...
async function probeHermesVersion(ssh, hermesPath) {
  try {
    const out = (await ssh.exec(`${REMOTE_ORGO_ENV_LOADER}${expandRemotePath(hermesPath)} --version 2>&1`)).trim()
    const lines = out.split('\n').map(line => line.trim())

    return lines.find(line => /^Hermes Agent v/i.test(line)) || lines[0] || ''
  } catch {
    return ''
  }
}
