...
test('classifySshError detects a Tailscale SSH identity check before generic auth', () => {
  const detail = [
    '# Tailscale SSH requires an additional check.',
    '# To authenticate, visit: https://login.tailscale.com/a/AbC_123-xYz'
  ].join('\n')

  assert.equal(classifySshError(detail), SSH_ERROR.REAUTH_REQUIRED)
  assert.equal(extractTailscaleSshCheckUrl(detail), 'https://login.tailscale.com/a/AbC_123-xYz')
  assert.equal(extractTailscaleSshCheckUrl('https://evil.example/a/AbC_123-xYz'), '')
})
...
test('open() preserves a partial Tailscale check prompt when ssh reaches the hard timeout', async () => {
  const checkPrompt = [
    '# Tailscale SSH requires an additional check.',
    '# To authenticate, visit: https://login.tailscale.com/a/fixture-token'
  ].join('\n')
  ...
  const spawnFn = scriptedSpawn(args => {
    ...
    return { hang: true, stderr: checkPrompt }
  })

  const conn = new SshConnection(
    { host: 'box', user: 'me' },
    { spawnFn, controlDir: '/tmp/d', connectTimeoutMs: 30 }
  )

  await assert.rejects(
    () => conn.open(),
    (err: any) => {
      assert.equal(err.kind, SSH_ERROR.REAUTH_REQUIRED)
      assert.equal(err.tailscaleAuthUrl, 'https://login.tailscale.com/a/fixture-token')
      assert.match(err.message, /Tailscale needs a quick identity check/i)
      assert.match(err.message, /https:\/\/login\.tailscale\.com\/a\/fixture-token/)
