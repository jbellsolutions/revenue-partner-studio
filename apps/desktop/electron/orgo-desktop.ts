import { requireStudioComputer } from './studio-policy'
...
// The official Orgo CLI stores profiles in ~/.orgo/credentials.json. A local,
// ad-hoc-signed desktop build gets a new macOS code-signing fingerprint after
// every rebuild, so asking safeStorage to decrypt an older Orgo key can block
// startup behind a Keychain authorization dialog. Reuse the already-authorized
// CLI credential when it exists; the parser is deliberately narrow and never
// returns any other profile metadata.
export function parseOrgoCliApiKey(raw: unknown): string {
  try {
    const value = typeof raw === 'string' ? JSON.parse(raw) : raw
    const current = String(value?.current || 'default').trim() || 'default'
    const profile = value?.profiles?.[current]
...
/** Once assigned, an app installation keeps the same Orgo computer. */
export function enforceOrgoComputerBinding(currentValue: unknown, requestedValue: unknown): string {
  const currentComputerId = normalizeOptionalOrgoComputerId(currentValue)
  const requestedComputerId = normalizeOptionalOrgoComputerId(requestedValue)

  if (
    currentComputerId &&
    requestedComputerId &&
    currentComputerId.toLowerCase() !== requestedComputerId.toLowerCase()
  ) {
    throw new OrgoDesktopError('invalid-config', 'This Revenue Partner Studio is already locked to its computer.')
  }

  return currentComputerId || requestedComputerId
}
...
export async function fetchOrgoDesktopSession(
  ...
  requireStudioComputer(computerId)
