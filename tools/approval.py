...
class _ApprovalEntry:
    ...
    def __init__(self, data: dict):
        ...
        self.data = {**data, "request_id": uuid.uuid4().hex}
...
def resolve_gateway_approval(session_key: str, choice: str,
    ...
    *reason* is an optional free-text explanation attached to an explicit
    ...
    if request_id and resolve_all:
        raise ValueError("A request-specific approval cannot resolve all requests")
    with _lock:
        ...
        for entry in list(queue):
            if entry.data.get('expires_at') is not None and entry.data['expires_at'] <= now:
                queue.remove(entry)
                entry.event.set()
        if not queue:
        ...
        for entry in targets:
            ...
            if choices and choice not in choices:
                raise ValueError("Approval choice is not offered for this action")
            if choice == "always" and entry.data.get("allow_permanent") is False:
                raise ValueError("Permanent approval is disabled for this action")
            if entry.data.get("smart_denied") and choice not in {"once", "deny"}:
                raise ValueError("This action allows only once or deny")
        if request_id:
...
def pending_gateway_approvals(session_key: str) -> list[dict]:
    """Copy still-live requests for reconnect; never replay resolved/expired entries."""
    with _lock:
        return [dict(entry.data) for entry in _gateway_queues.get(session_key, [])
                if not entry.event.is_set() and (entry.data.get('expires_at') is None or entry.data['expires_at'] > time.time())]
...
def _await_gateway_decision(session_key: str, notify_cb, approval_data: dict,
    ...
    entry = _ApprovalEntry({**approval_data, "expires_at": time.time() + max(_get_approval_timeout(), 0)})
    with _lock:
...
def request_elicitation_consent(
    ...
    *,
    ...
    """
    # Keep approvals.mode=off truthful across every tool transport. Terminal
    # and execute_code already bypass their interactive gates in this mode;
    # without the same policy here, write-capable MCP tools still produce the
    # approval dialogs the operator explicitly disabled. URL-mode MCP
    # elicitations are rejected before reaching this function, so OAuth and
    # payment redirects retain their separate fail-closed boundary.
    if (
        _YOLO_MODE_FROZEN
        ...
        logger.info(
            "MCP consent auto-accepted by explicit no-prompt approval policy "
            "(surface=%s)",
            surface,
        )
        return "accept"
