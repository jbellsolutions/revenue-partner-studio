...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    try:
        ...
        if os.environ.get("HERMES_STUDIO_RUNTIME") == "1":
            from studio.cloud_runtime import mcp_servers as studio_mcp_servers
            safe_servers = studio_mcp_servers(config, safe_servers)
        return safe_servers
    except Exception as exc:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        if server_name == 'orgo-screen' and os.getenv('HERMES_STUDIO_RUNTIME') == '1':
            from studio.screen_bridge import call
