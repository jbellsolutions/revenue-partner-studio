...
def _forward_context_key(profile: str, slug: str, peer: str, context_id: str) -> str:
    """Authenticated caller + exact context own a thread, not its display slug.

    Sanitizing/truncating IDs is lossy. Hash the full tuple so two peers or
    contexts cannot inherit each other's history, including after restart.
    Never fall back to old titles lacking authenticated ownership.
    """
    import hashlib
    encoded = json.dumps([profile, slug, peer, context_id], ensure_ascii=True).encode()
...
class A2ARequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: N802
        ...
        if route.get("error"):
            ...
            return
        # Request-local copy: authenticated identity must scope every task RPC,
        # not only message/send. Never mutate the shared route across threads.
        agent = {**route["agent"], "_authenticated_peer": identity}
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _scope_for_agent(self, agent: Optional[dict]) -> tuple[str, str]:
        agent = agent or self._agents[""]
        if "_authenticated_peer" in agent:
            return str(agent.get("slug") or ""), _forward_context_key(
                str(agent.get("profile") or "default"), str(agent.get("slug") or ""),
                str(agent["_authenticated_peer"]), str(agent.get("tenant") or ""),
            )
    ...
    def _prepare_task(self, params: dict, peer: str, agent: Optional[dict] = None) -> tuple[Optional[dict], Optional[dict]]:
        ...
        private_context = _forward_context_key(
            str(agent.get("profile") or "default"), str(agent.get("slug") or ""), peer, context_id,
        )
        task_id = protocol.new_task_id()

        # Anti-loop ping-pong protection
        turn = self._turns.track(private_context)
        ...
        protocol.persist_message(private_context, "user", text, task_id)
        ...
        if not agent.get("local", True):
            ...
            protocol.persist_message(private_context, "agent", reply, task_id)
        ...
        fut = self._add_pending(task_id, private_context)

        event = MessageEvent(
            text=framed,
            message_type=MessageType.TEXT,
            source=self.build_source(
                chat_id=private_context,
    ...
    def _lookup_forward_session(self, profile: str, title: str) -> str:
        ...
        try:
            ...
            row = con.execute(
                "SELECT id FROM sessions WHERE source = 'a2a' AND title = ? ORDER BY started_at DESC LIMIT 1",
                (title,),
            ).fetchone()
            con.close()
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        ...
        scoped_context = _forward_context_key(profile or "default", slug, peer, context_id)
        session_title = f"a2a-{_safe_context_slug(slug, 32)}-{scoped_context}"
        key = (profile or "default", slug, scoped_context)
        ...
        with lock:
            ...
            # The CLI emits this exact receipt on stderr in quiet mode.
            # Guessing the newest DB row can select another concurrent
            # caller's session. Older CLIs without a receipt fail closed:
            # first contact isn't cached. Follow compression receipts on
            # resumed turns too, rather than reopening an archived parent.
            receipts = re.findall(r"^session_id:\s*([A-Za-z0-9_-]+)\s*$", proc.stderr or "", re.MULTILINE)
            if receipts:
                session_id = receipts[-1]
    ...
    def _finalize_task(self, pending: dict, state: str, reply: str) -> tuple[str, str]:
        ...
        protocol.persist_message(pending.get("private_context", context_id), "agent", reply, task_id)
        security.audit("outbound", peer, task_id, reply)
    ...
    def _rpc_tasks_cancel(self, req_id: Any, params: dict, agent: Optional[dict] = None) -> dict:
        ...
        target = agent or self._agents[""]
        self._turns.reset(_forward_context_key(
            str(target.get("profile") or "default"), str(target.get("slug") or ""),
            str(rec.get("peer") or ""), rec["context_id"],
        ))
